...
def test_multistage_dispatch_records_derived_handoffs_and_cumulative_latency():
    sim = build_game_application()._simulation
    _allow_external_transport(sim)
    ...
    _advance_logistics(sim, sim.day, (requirement,))
    flow = next(row for row in sim.logistics.cargo_flows.values() if row.demand_id == requirement.id)
    assert flow.service_ids == expected_services
    assert flow.service_destinations == expected_destinations
    assert flow.ready_day - flow.departure_day == expected_latency
...
def test_cargo_arrival_waits_for_inventory_admission():
    ...
    projected = next(row for row in app.query(GetCargoFlows()).items if row.id == str(flow.id))
