...
def _route_between(sim, origin, destination, prefix=DERIVED_SURFACE_ACCESS_ROUTE_PREFIX):
    matches = [
        route_id for route_id, route in sim.transport.routes.items()
...
def test_derived_surface_route_uses_real_gateway_facility_cells():
    ...
    geometry = sim.transport.route_geometry(route_id)
...
def test_gateway_position_changes_route_geometry_without_changing_location_identity():
    ...
    before = sim.transport.route_geometry(before_id)
    ...
    after = sim.transport.route_geometry(after_id)
...
def test_surface_transport_latency_uses_endpoint_distance_and_vehicle_speed():
    ...
    forward = sim.transport.routes[forward_id]

    distance = sim.transport.route_geometry(forward_id).distance_km
    assert distance is not None
    expected_days = max(1, math.ceil(distance / 25.0))
    assert sim.transport.performance_route_transit_days(forward, vehicle.performance) == expected_days
    plan = sim.transport.transport_service_plan_for(vehicle.id, a, b, path=(forward_id,))
...
def test_player_founded_location_gets_orbit_routes_only_after_real_gateway_exists():
    ...
    assert sim.transport.route_geometry(down).destination.surface_cell_id == ids.MOON_CELL_FARSIDE_HIGHLANDS
    assert sim.transport.route_geometry(up).origin.surface_cell_id == ids.MOON_CELL_FARSIDE_HIGHLANDS
...
def test_surface_transport_distance_limit_is_physical_blocker():
    ...
    b, _ = _location_with_gateway(sim, "b", ids.MOON_CELL_NEARSIDE_MARE)
    route_id = _route_between(sim, a, b)
    distance = sim.transport.route_geometry(route_id).distance_km
    ...
    sim.transport.vehicle_defs[vehicle.id] = vehicle
    assert any("surface_transport:distance:" in f for f in sim.transport.vehicle_route_physical_failures(route_id, vehicle.id))
...
def test_derived_surface_route_is_rebuilt_after_save_load(tmp_path):
    ...
    assert route_id in loaded._simulation.transport.routes
    geometry = loaded._simulation.transport.route_geometry(route_id)
