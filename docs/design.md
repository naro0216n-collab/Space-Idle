...
研究の発展段階は、Technology DAGの成熟度とゲーム世界における宇宙活動の変化をプレイヤーへ可視化するための表示区分とする。段階そのものを研究開始条件、Technology Unlock条件、全研究取得条件として使用しない。研究開始可否は各Research Definitionが持つ具体的なprerequisiteから決まり、同じ表示段階の全項目を取得することを要求しない。

表示名の基準は次の通りとする。第1〜4段階は現在具体化する範囲、第5段階以降は将来の発展方向を共有するための参考名であり、段階数の上限を意味しない。
...
研究の専門区分も研究可否を規定する壁にはしない。推進、電力、熱、材料、通信、計算、航法、自律制御、観測、資源、建設、生命維持、生命科学、食料、居住、保守等は、Technology DAGを理解しやすく表示・整理するための分類である。技術課題の分解に応じて区分を細分化・統合でき、すべての区分へ同数の研究を配置しない。特定分野だけを恒常的に細分化し、他分野を巨大な一研究へ圧縮しない。

各技術系列は、可能な限り同じ技術課題が段階を越えてどのように成熟したかを追える縦方向の連続性を持たせる。基礎現象の理解・計測、基本機構の成立、長時間または高負荷条件への対応、劣化・故障の理解、反復利用・閉ループ化、自律・統合運用等は典型的な発展方向だが、すべての系列を同じ段数へ強制しない。段階4を技術的完成点とせず、成熟した研究は第5段階以降の産業、自律運用、物質循環、惑星間ネットワーク、極端環境技術等へ前提として接続できる構造にする。

Technology DAGは区分内の縦系列だけでなく、実際の技術成立に必要な区分横断prerequisiteを持つ。例えば推進が材料・熱制御・計算・計測へ、生命維持が電力・熱・微生物学・計測へ依存するように、技術的因果関係がある場合は区分を跨いで接続する。一方、横断依存を増やすこと自体を目的にせず、一つの合理的経路を選んでも進行可能な複数の技術経路を保つ。

### 13.2 研究項目の設定原則

研究項目名は、目的地、用途、製品世代、抽象的な性能Tierではなく、何を理解・制御・製造・運用できるようにした研究なのかを表す。Location名を冠した「月面○○」「火星○○」、成果だけを表す「大型○○」「大規模○○」「高性能○○」「次世代○○」、内容を示さない `II` / `III` 等を研究内容の代わりに用いない。

一方、「高温」「高圧」「極低温」「高電圧」「長期」のような語が、対象となる物理領域、負荷条件、時間尺度を具体的に示す場合は使用できる。名称の具体性は固有Locationへ依存することではなく、一般化可能な工学・科学上の問題を明示することで確保する。
...
製品名、最終的な運用状態、特定Locationでの達成状態そのものを研究項目へ置かず、それらを成立させる具体的な研究課題へ分解する。逆に、部品寸法や単一部品の微細な改良まで無制限に研究項目へ分解せず、独立したプレイヤー判断または後続技術への因果関係を持たない差はContent内の性能差として扱う。

具体的な研究項目名、各項目の表示段階、専門区分、prerequisiteはContent Definitionとして構成し、本書では個別研究一覧を正準化しない。Technology DAGはacyclicでなければならず、同一系列の縦方向の発展と区分横断依存の双方を保持する。

一本道にはせず、推進・発電・自律化・有人／無人運用等について複数の合理的な研究経路を持たせる。すべての研究取得を進行条件にせず、選択した技術体系に応じて未取得系列が残ることを正常な状態とする。
