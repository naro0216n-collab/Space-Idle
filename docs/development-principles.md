...
各payload世代では、helperがpayload fieldごとの正準Git blob OIDを事前計算し、そのOIDを世代indexへ記録する一方、GitHubへのuploadは個別 `create_blob` ではなくinline `content` を持つtree elementとして行う。`GitHub.create_tree` は同一call内でblob生成とtree組立を行うため、handoff field数が増えてもhard ceiling以内なら送信は1回である。hard ceilingを超える場合だけ、直前に返されたunreferenced tree SHAを次callのbase treeとして使い、最終treeまで連続して組み立てる。tree生成後は返却tree SHAを `connector-tree` に渡し、helperが次tree packetまたはcommit packetを生成する。最終commitの返却SHAを `connector-update` へ渡し、固定 `publish` branchをnon-forceで一度だけ進める。これによりhandoff field数やtree call数に関係なく、payload part・index・request/retry triggerは一つのcommitで同時に可視化され、部分的なbranch状態を作らない。blob返却SHAの中継や本文SHAの手動照合は行わない。

payload handoffまたはGateway検証の失敗はactive transactionを作り直さず、同一request ID内で次世代へ収束させる。`connector-repair` はremote blob SHAやpart indexを入力にせず、元の正準payloadからretry世代を再生成する。repair開始時に `publish` HEAD/treeを一度だけ観測し、それが直前世代の基点または直前世代を原子的に反映したcommit/treeのどちらかであることをhelperが検証する。次世代もinline tree elementを144 KiB hard ceiling内へpackingし、必要なら複数のunreferenced tree callを経た後、一つのcommitへまとめてnon-force ref updateを一度だけ行う。これにより壊れたremote fileの上書き、stale SHA、手動part特定、partごとのbranch commitを必要としない。再prepare、再plan、固定小chunkへの切替、payload本文やSHAの目視検証、inline requestへの切替は標準retryに含めない。
...
Gatewayは初回requestまたは同一request IDのretry envelopeを含む原子的transport commitを受けたら、request固有payload directoryからtriggerが要求する世代以上の最大世代indexを選び、indexの世代・part順序・branch上のpart path・各Git blob OID・part文字数・payload SHA-256を機械検証したうえでbundle、publish commit、parent/base、target tree、直前remote HEADまで検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新せず、失敗世代とpartをログへ明示する。ChatGPT側の正常系・repair系に個別blob再取得、payload本文やSHAの目視比較、blob返却SHAによる内容正当化を置かない。tree SHAとcommit SHAは直前のGit data action結果として対応するhelper stageへだけ渡し、手動でtree内容やcommit関係を再構成しない。ローカルのpublish state更新はreceiptとmanifestを照合して行い、manifestが保持するlocal target commitを正本とする。receipt待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
