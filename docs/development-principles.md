...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。repository、target ref、manifest path、plan directoryは通常CLIの選択肢にせず、repo-localな単一active transaction領域からhelperが一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle payloadは独立Git blobとしてmaterializeし、Connector actionへ渡すcompact JSON実引数bytesを固定Connector profileで評価して、必要な場合だけ最少数へ自動分割する。call budgetはtransport実装の一部としてhelper内に固定し、request manifestや通常CLIの調整項目にしない。helperはupload packet群と最終request packetを同じplanで決定論的に生成し、requestはpayloadを構成するGit blob OIDを順序付きで直接保持する。Connector返却SHAを後続helper commandへ中継せず、全upload成功後にrequestを送信する。content転送が変化または欠落して期待blob OIDがmaterializeされなければGatewayが対象blobを取得・再検証できないためpublish前に停止する。packet転送の異常はtransport integrity failureとして扱い、同じrequestを任意の小さいbudgetへ再分割する運用には遷移しない。payloadをrequest本文へ直接埋め込むinline経路や、完全性確認だけを目的とするpayload root treeは設けない。
...
Gatewayはrequestを受けたら、順序付き各Git blob OID、payload SHA-256、bundle、publish commit、parent/base、target tree、直前remote HEADを機械検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新しない。ChatGPT側の正常系に個別blob再取得、SHA目視比較、Connector返却SHAの中継、手動commit/ref更新を置かない。ローカルのpublish state更新もreceiptとmanifestを照合して行い、manifestが保持するlocal target commitを正本とする。receipt待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
