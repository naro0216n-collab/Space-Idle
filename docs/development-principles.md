...
Connector actionの種類と順序もhelperの出力を実行契約とする。`gateway-begin`はGit bundle Base64を8 KiBのtransport chunkへ決定論的に分け、各chunkを`GitHub.create_tree(content=...)`で匿名Git objectとしてmaterializeするactionと、全chunkのhelper事前計算blob OIDだけを参照するSHA-only root `GitHub.create_tree` actionを提示する。8 KiBはConnector call上限ではなくtransportの検証・再送粒度であり、Connector本来の96 KiB call容量とは分離する。各chunk uploadの返却tree SHAは後続入力にせず、転記が壊れて期待blobが生成されなければroot tree作成自体が成立しない構造にする。root tree作成成功時だけ、その短い返却SHAをhelperが事前計算root tree OIDと照合し、一致後に短い最終`GitHub.create_file` actionを生成する。全文再取得・全文比較、個別upload SHAの中継、adaptive blob分割を正常系に置かない。ChatGPTはactionを選び直したり、chunk境界・Base64・OID・root tree・request本文を再構成しない。
...
Gatewayはrequestを受けたら、payload root tree OID、各chunk blob OID、payload SHA-256、bundle、publish commit、parent/base、target tree、直前remote HEADを機械検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新しない。正常系に個別upload返却SHAの中継、SHA目視比較、手動commit/ref更新を置かない。ローカルpublish stateは`gateway-complete`がactive sessionの決定論的publish commitと観測target HEADを照合した場合だけ更新し、manifestが保持するlocal target commitを正本とする。Gateway処理待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
