...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。source-snapshot pathは復元repoの `origin` が指す `repository.bundle` からhelperが一意に解決し、repository、target ref、manifest path、plan directoryと同様に通常CLIの選択肢にしない。repo-localな単一active transaction領域からhelperが状態を一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle Base64はConnector actionの固定144 KiB上限内なら単一partとし、上限を超える場合だけ実action引数bytesから必要最少数へ自動分割する。chunk sizeを転送信頼性の調整値として扱わず、request manifestや通常CLIから変更できるbudgetも設けない。各partは固定 `publish` branchのrequest固有directoryへ `GitHub.create_file` で作成し、helperは各blob OIDとrequest固有payload subtree OIDを事前計算したうえで最終request packetを生成する。これによりpayloadはbranch上のcommitから到達可能なtreeとしてGateway credentialから参照できる。Connector返却SHAを後続helper commandへ採用せず、requestは事前計算OIDだけを使用する。

payload転送の失敗はactive transactionを作り直さず、失敗箇所だけを修復する。remote fileが存在しない場合は同じ生成済みupload packetを再実行する。remote fileが存在するが期待blob OIDと一致しない場合は、観測したremote blob SHAを検証入力としてhelperへ渡し、元の生成済みupload packetのcontent/pathから `GitHub.update_file` repair packetを生成する。repair後は当該pathを一度だけ再取得して期待blob OIDへの一致を確認する。request送信前の失敗なら既存request packetへ続行し、request送信後にGateway payload検証で停止した場合は新しいrequestを作らず同じ失敗Gateway jobをrerunする。再prepare、再plan、任意の小さいbudgetへの再分割、payload本文の手動再構成、inline requestへの切替は標準retryに含めない。

通常Publish Gatewayの権限境界はgame/source publishに限定し、`.github/workflows/**` を含むtargetはrequest生成前に識別してworkflow maintenance経路へ分離する。workflow maintenanceは、workflow更新権限を明示的に持つ認証経路だけで実行し、通常Gatewayへ権限を常設追加して兼用しない。

Gatewayはrequestを受けたら、payload subtree OID、tree entry順序、各Git blob OID、payload SHA-256、bundle、publish commit、parent/base、target tree、直前remote HEADを機械検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新しない。ChatGPT側の正常系に個別blob再取得、SHA目視比較、blob/tree返却SHAの後続入力への中継、手動commit/ref更新を置かない。失敗時のremote取得は不一致partの識別とrepair後の一度の確認に限定する。ローカルのpublish state更新もreceiptとmanifestを照合して行い、manifestが保持するlocal target commitを正本とする。receipt待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
