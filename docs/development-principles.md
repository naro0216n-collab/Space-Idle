...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。repository、target ref、manifest path、plan directoryは通常CLIの選択肢にせず、repo-localな単一active transaction領域からhelperが一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle Base64はConnector actionの固定物理上限内なら単一Git blobとし、上限を超える場合だけ実action引数bytesから必要最少数へ自動分割する。chunk sizeを転送信頼性の調整値として扱わず、request manifestや通常CLIから変更できるbudgetも設けない。helperはblob upload packet群、事前計算blob OIDを順序付きで参照するpayload root tree packet、事前計算tree OIDを参照する最終request packetを一度に決定論的に生成する。全blob upload後に `GitHub.create_tree` を実行し、期待blob群がGitHub object storeにmaterializeされていなければrequest送信前に停止する。tree返却SHAを後続helper commandへ中継せず、requestは事前計算tree OIDだけを使用する。payload転送の異常は同じ生成済みpacketの再実行で扱い、任意の小さいbudgetへの再分割、payload本文の手動再構成、inline requestへの切替は標準経路に含めない。
