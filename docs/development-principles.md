...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。source-snapshot pathは復元repoの `origin` が指す `repository.bundle` からhelperが一意に解決し、repository、target ref、manifest path、plan directoryと同様に通常CLIの選択肢にしない。repo-localな単一active transaction領域からhelperが状態を一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle Base64はConnector actionの固定物理上限内なら単一Git blobとし、上限を超える場合だけ実action引数bytesから必要最少数へ自動分割する。chunk sizeを転送信頼性の調整値として扱わず、request manifestや通常CLIから変更できるbudgetも設けない。helperはblob upload packet群、事前計算blob OIDを順序付きで参照するpayload root tree packet、事前計算tree OIDを参照する最終request packetを一度に決定論的に生成する。全blob upload後に `GitHub.create_tree` を実行し、期待blob群がGitHub object storeにmaterializeされていなければrequest送信前に停止する。tree返却SHAを後続helper commandへ中継せず、requestは事前計算tree OIDだけを使用する。payload転送の異常は同じ生成済みpacketの再実行で扱い、任意の小さいbudgetへの再分割、payload本文の手動再構成、inline requestへの切替は標準経路に含めない。
...
Gatewayはrequestを受けたら、payload root tree OID、tree entry順序、各Git blob OID、payload SHA-256、bundle、publish commit、parent/base、target tree、直前remote HEADを機械検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新しない。ChatGPT側の正常系に個別blob再取得、SHA目視比較、blob/tree返却SHAの後続入力への中継、手動commit/ref更新を置かない。ローカルのpublish state更新もreceiptとmanifestを照合して行い、manifestが保持するlocal target commitを正本とする。receipt待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
