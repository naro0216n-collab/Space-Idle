...
native Git transportが利用できない環境では、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。Connector actionへ渡す実引数bytesを基準に、helperは最少call数でpayload root treeをmaterializeする。bundle payloadが1回のtree作成callに収まる場合はroot tree entryのcontentとして直接materializeし、収まらない場合だけ最少数の独立Git blobへ自動分割して並列送信した後にroot treeを組み立てる。helperは各blob OIDとpayload root tree OIDを事前計算し、materialization packetと最終request packetを同時に生成する。blob/tree作成の返却SHAは後続stepへ渡さず、成功/失敗だけをtransport進行条件とする。payloadをrequest本文へ直接埋め込むinline経路は設けない。
