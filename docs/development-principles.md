...
GitHubへのtransport方式はゲーム実装の構造やcommit境界を決める根拠にしない。publish対象はcommitted target treeであり、その後にworking treeで別作業を続けていても対象commitへ未commit内容を混入させない。認証済みnative Gitを利用できる環境では、記録済みremote HEAD/treeを基点としてlocal target treeを指すtransport commitを生成し、non-force push後にremote ref/treeを再検証する経路を優先する。local historyとremote historyのcommit SHAが異なっても、publish都合でlocal historyをrebase・resetせずtree同一性を保証する。

native Git transportが利用できない環境では、記録済みremote commitを親、local target treeをtreeに持つ決定論的なpublish commitをGit bundleへ格納してPublish Gatewayへ渡す。Connectorがlocal binary/file uploadを持たない場合はbundleをbounded Base64 chunkへ分離するが、chunk数をそのままtool call数へ対応させない。publish直前にtarget branch HEADを一度確認し、現在の `publish` branch commit/treeも一度だけ取得する。helperに両者を渡してmanifest baseとの一致を機械確認したうえで、実際にConnector actionへ渡す引数のシリアライズbytesを基準に、全chunkとmanifestをpublish root treeへ直接接続する1 callを最初に選ぶ。収まらない場合だけ最少数の独立groupへ自動分割して並列materializeし、最終root treeではローカル算出済みblob OIDを直接参照する。正常経路に独立chunks-tree assemblyやupload返却SHAの中継を置かない。最終chunks tree OID、payload digest、bundle内commitのparent/tree、remote HEAD、publish後remote tree、receiptはGitHub側とGatewayで機械検証し、正常系でuploadごとのSHAを人手転記・目視比較しない。Connectorがbinary/file uploadを提供する場合はchunk層を外して同じbundleを直接送る。不完全な転送やremote HEAD不一致では対象branchを更新しない。
