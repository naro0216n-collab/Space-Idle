...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。repository、target ref、manifest path、plan directoryは通常CLIの選択肢にせず、repo-localな単一active transaction領域からhelperが一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle payloadは独立Git blobとしてmaterializeし、Connector actionへ渡すcompact JSON実引数bytesを固定Connector profileで評価して、必要な場合だけ最少数へ自動分割する。call budgetはtransport実装の一部としてhelper内に固定し、request manifestや通常CLIの調整項目にしない。connector transportは `payload plan -> blob uploads -> root packet -> root creation -> request packet -> request submission` の一方向状態遷移とし、helperは現在stageで実行可能なpacketだけを生成する。blob upload完了後にroot packetを生成し、root作成結果が事前計算OIDと一致した場合だけ最終request packetを生成する。返却OIDは後続Git objectの入力値として採用せず、事前計算OIDとの機械的な成立確認にだけ使う。content転送が変化して期待blob OIDがmaterializeされなければroot作成で停止し、requestは生成されない。packet転送の異常はtransport integrity failureとして扱い、同じrequestを任意の小さいbudgetへ再分割する運用には遷移しない。payloadをrequest本文へ直接埋め込むinline経路は設けない。
