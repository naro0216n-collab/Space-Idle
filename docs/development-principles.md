...
Connector actionの種類と順序もhelperの出力を実行契約とする。`gateway-begin`はConnector本来のcall容量を上限として最小数の`GitHub.create_blob` upload actionを提示する。helper packet自体を現在のexecution bridgeが忠実に引き渡せない場合は、不完全な内容を送信してSHA不一致を意図的に起こすのではなく、remote write前に`gateway-refine`を使う。helperが最大の未確認partだけを一段二分し、part選択・境界を所有する。各uploadの返却SHAはhelperが事前計算したGit blob OIDと照合し、一致したpartを確定する。不一致partだけをhelperが二分して再発行し、成功済みpartは再送しない。この検証付きadaptive splitを必要なpartだけ反復し、全partのOID一致後にだけ短い最終`GitHub.create_file` actionを生成する。全文再取得・全文比較や経験的なbridge固定上限を正常系の判断材料にしない。ChatGPTはactionを選び直したり、part境界・Base64・OID・request本文を再構成しない。
