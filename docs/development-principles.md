...
現在のConnector実行環境では、publish対象を現在の `HEAD` commitへ固定し、記録済みremote commitを親、local target treeをtreeに持つ決定論的commitをGit bundleへ格納し、Publish Gatewayへ渡す。source-snapshot pathは復元repoの `origin` が指す `repository.bundle` からhelperが一意に解決し、repository、target ref、manifest path、plan directoryと同様に通常CLIの選択肢にしない。repo-localな単一active transaction領域からhelperが状態を一意に解決する。active transactionがある間は別requestを開始せず、成功receiptの記録後だけ閉じる。publish直前のChatGPT側確認は対象branch HEADの一度だけとし、それ以外のSHA整合性確認はhelperとGatewayへ移す。bundle Base64の初期世代はConnector actionの固定144 KiB上限をhard ceilingとして実action引数bytesから必要最少数へ自動分割する。request manifestや通常CLIから任意のchunk budgetを指定できる経路は設けない。各世代は固定 `publish` branchのrequest固有directoryへ世代別pathで `GitHub.create_file` し、世代indexにpart順序、各blob OID、文字数、payload SHA-256を記録する。最終requestはrequest固有payload directoryだけを参照し、特定世代やsubtree OIDへ固定しない。生成packetの `action_args` はコピペで対応するConnector callへ渡す。Connector返却SHAを後続helper commandへ採用せず、payloadの成立可否はGatewayがindex・branch上のpath・Git blob OID・payload digestから機械検証する。

payload転送の失敗はactive transactionを作り直さず、同一request ID内でappend-onlyな次世代payloadを生成して収束させる。`connector-repair` はremote blob SHAやpart indexを入力にせず、直前世代で実際に生成された最大Connector call bytesを基準に次世代のcall envelopeを縮小し、元の正準payloadからpartを再生成する。次世代は新しい世代別pathとindexを `GitHub.create_file` するため、壊れたremote fileの上書き、stale SHA、手動part特定を必要としない。各repair世代では同じrequest ID・base・target tree・publish commitを持つappend-onlyなretry envelopeを生成し、その世代を下限としてGateway検証を起動する。これにより失敗jobの状態や古いrequest fileの書換えへ依存せず、active transactionの同一性を維持したまま再検証できる。再prepare、再plan、固定小chunkへの切替、payload本文の手動再構成、inline requestへの切替は標準retryに含めない。
...
Gatewayは初回requestまたは同一request IDのretry envelopeを受けたら、request固有payload directoryからretry envelopeが要求する世代以上の最大世代indexを選び、indexの世代・part順序・branch上のpart path・各Git blob OID・part文字数・payload SHA-256を機械検証したうえでbundle、publish commit、parent/base、target tree、直前remote HEADまで検証する。すべて一致した場合だけ対象branchへexact commitをnon-force publishし、remote ref/treeを再確認してreceiptを記録し、Fast CIを起動する。不一致時は対象branchを更新せず、失敗世代とpartをログへ明示する。ChatGPT側の正常系・repair系に個別blob再取得、SHA目視比較、blob/tree返却SHAの後続入力への中継、手動commit/ref更新を置かない。ローカルのpublish state更新もreceiptとmanifestを照合して行い、manifestが保持するlocal target commitを正本とする。receipt待ちの間にlocal HEADが進んでも、過去checkpointのSHAを人手で引き渡さない。
