...
from .shared import CelestialBodyId, DefinitionId, ProjectId, SpatialNodeId, SurfaceCellId
...
def restore_founding(sim: Any, data: dict[str, Any]) -> None:
    ...
    for row in data.get("projects", []):
        p = LocationFoundingProject(
            ...
            departure_day=None if row.get("departure_day") is None else int(row["departure_day"]),
            arrival_day=None if row.get("arrival_day") is None else int(row["arrival_day"]),
...
def validate_runtime(sim: Any) -> None:
    ...
    for project_id, p in service.projects.items():
        ...
        _require(p.vehicle_definition_id in sim.logistics.vehicle_defs, f"founding project vehicle missing: {project_id}")
        ...
        if active:
            ...
            _require(reservation_id in sim.logistics.fleet_reservations, f"active founding missing fleet reservation: {project_id}")
            if reservation_id in sim.logistics.fleet_reservations and package is not None:
                r = sim.logistics.fleet_reservations[reservation_id]
            ...
            _require(reservation_id not in sim.logistics.fleet_reservations, f"inactive founding retains fleet reservation: {project_id}")
        if p.status is FoundingStatus.PREPARING:
            ...
            _require(p.departure_day is not None and p.arrival_day is not None, f"deploying founding lacks transit dates: {project_id}")
