(() => {
  ...
  function constructionPlanControls(prefix,{draftScope=prefix,policyOptions=[],sourceOptions=[],selectedPolicy='mixed',selectedSource=null,disabled=false}={}){
    ...
    return `<div class="form-row"><label>優先度<select id="${prefix}PriorityInput" data-draft-key="${esc(draftScope)}:priority" ${disabledAttr}>${priorityOptions(3)}</select></label><label>調達方針<select id="${prefix}SourcingPolicy" data-draft-key="${esc(draftScope)}:sourcing" ${disabledAttr}>${policyRows}</select></label><label>優先供給元<select id="${prefix}ImportSource" data-draft-key="${esc(draftScope)}:source" ${disabledAttr}><option value="" ${selectedSource==null?'selected':''}>物流Laneから自動選択</option>${sourceRows}</select></label></div>`;
  }
  ...
  function surfacePlanControls(scope,option,disabled=false){
    ...
    return `<div class="form-row surface-plan-controls"><label>優先度<select data-surface-plan-priority data-draft-key="${esc(scope)}:priority" ${d}>${priorityOptions(3)}</select></label><label>調達方針<select data-surface-plan-policy data-draft-key="${esc(scope)}:sourcing" ${d}>${policy}</select></label><label>優先供給元<select data-surface-plan-source data-draft-key="${esc(scope)}:source" ${d}><option value="">物流Laneから自動選択</option>${sources}</select></label></div>`;
  }
  ...
  const foundingPlanControls=(scope,option,disabled=false)=>{
    ...
    const d=disabled?'disabled':'';
    return `<div class="form-row"><label>優先度<select data-founding-priority data-draft-key="${esc(scope)}:priority" ${d}>${priorityOptions(3)}</select></label><label>優先供給元<select data-founding-source data-draft-key="${esc(scope)}:source" ${d}><option value="">物流Laneから自動選択</option>${sources}</select></label></div>`;
  };
  ...
  function renderProjectInspector(id){
    ...
    const targetRows=[['ID',esc(p.id)],['種別',esc(target)],['状態',esc(stateLabels[p.status]||p.status||(p.paused?'paused':'active'))],['優先度',esc(p.priority??'—')],['施工充足',pct(p.construction_fulfillment??1)],['調達方針',esc(sourcingPolicyName(p.sourcing_policy))],['優先供給元',p.import_source_id?esc(locationName(p.import_source_id)):'物流Laneから自動選択'],['工数',`${fmt(p.progress??p.construction_done??0)}/${fmt(p.construction_required??0)}`]];
