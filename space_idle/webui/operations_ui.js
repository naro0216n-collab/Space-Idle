(() => {
  ...
  const foundingPlanControls=(scope,option,disabled=false)=>{
    ...
    const d=disabled?'disabled':'';
    return `<div class="form-row"><label>優先度<input type="number" step="1" value="50" data-founding-priority data-draft-key="${esc(scope)}:priority" ${d}></label><label>優先供給元<select data-founding-source data-draft-key="${esc(scope)}:source" ${d}><option value="">物流Laneから自動選択</option>${sources}</select></label></div>`;
  };
  ...
  function renderProjectInspector(id){
    ...
    const projectControls=foundingProject
      ? `<div class="action-stack"><button type="button" data-command="${p.paused?'ResumeFounding':'PauseFounding'}" data-project-id="${esc(p.id)}" ${settingsDisabled}>${p.paused?'準備再開':'準備停止'}</button><div class="form-row"><label>優先度<input id="projectPriorityInput" type="number" step="1" value="${esc(p.priority??50)}" ${settingsDisabled}></label><button type="button" data-set-founding-priority="${esc(p.id)}" ${settingsDisabled}>優先度を適用</button></div><button type="button" class="danger-button" data-command="CancelFounding" data-project-id="${esc(p.id)}" ${settingsDisabled}>Deployment取消</button></div>`
  ...
  function renderSurveyInspector(id){
    ...
    const blockers=s.blockers||[],priorityEditable=s.can_start||s.can_set_priority,priority=s.priority??50;
