(() => {
  ...
  function surveyComparisonHtml(campaign){
    return comparisonSurfaceHtml({
      ...
      headingHtml:(row)=>`<strong>${esc(row.provider_display_name||'調査Provider')}</strong><span>${esc(row.observation_mode_display_name||'観測方式')}</span><small>${esc(locationName(row.provider_operational_node_id))}</small>`,
  ...
  function researchComparisonHtml(research){
    return comparisonSurfaceHtml({
      ...
      promptText:'実行地点から2〜4候補を比較に追加すると、Fleet拘束・Resource・Service供給・所要時間をApplicationの共通軸で確認できます。',
  ...
  function detailedForecastHtml(){
    ...
    const horizonButtons=[['SHORT_TERM','短期'],['MEDIUM_TERM','中期'],['STEADY_STATE','Steady-state']].map(([value,label])=>`<button type="button" data-detailed-forecast-horizon="${value}" class="${detailedForecastHorizon===value?'is-active':''}" aria-pressed="${detailedForecastHorizon===value?'true':'false'}">${label}</button>`).join('');
    const controls=`<div class="detailed-forecast-controls"><div class="segmented-control" role="group" aria-label="Detailed Forecast horizon">${horizonButtons}</div><button type="button" class="primary" data-run-detailed-forecast ${detailedForecastLoading?'disabled':''}>${detailedForecastLoading?'予測中':'Detailed Forecastを実行'}</button></div>`;
    if(!forecast)return `${controls}<div class="empty-state">広域予測は明示実行時だけ計算します。将来Inventory、downstream state、multi-hop LogisticsをFast Previewから分離して確認できます。</div>`;
    const steadyLabels={stable:'収束/均衡',accumulating:'蓄積傾向',depleting:'枯渇傾向'};
    const inventory=(forecast.inventory||[]).map((row)=>`<div class="detail-card"><div class="mode-title"><span>${esc(row.display_name||resourceName(row.resource_id))}</span><span class="badge">${esc(steadyLabels[row.steady_state]||A.userFacingText(row.steady_state))}</span></div><div class="dependency-metrics"><span><small>現在</small><strong>${fmt(row.current_amount,2)} ${esc(row.unit)}</strong></span><span><small>予測</small><strong>${fmt(row.projected_amount,2)} ${esc(row.unit)}</strong></span><span><small>変化</small><strong>${signed(row.delta_amount)} ${esc(row.unit)}</strong></span><span><small>生産</small><strong>${fmt(row.projected_production_per_day,2)}/日</strong></span><span><small>消費</small><strong>${fmt(row.projected_consumption_per_day,2)}/日</strong></span><span><small>外部依存</small><strong>${fmt(row.projected_external_dependency_per_day,2)}/日</strong></span></div></div>`).join('')||'<div class="empty-state">このhorizonで変化するInventoryはありません。</div>';
    const impacts=(forecast.downstream_impacts||[]).map((row)=>`<div class="detail-card"><div class="mode-title"><span>${esc(row.display_name)}</span><span class="badge">${esc(A.userFacingText(row.kind))}</span></div><div class="cell-sub">${esc(A.userFacingText(row.current_state))} → ${esc(A.userFacingText(row.projected_state))}</div></div>`).join('')||'<div class="empty-state">このhorizonで主要な案件・研究状態遷移はありません。</div>';
    const logistics=(forecast.logistics_impacts||[]).map((row)=>`<div class="detail-card"><div class="mode-title"><span>${esc(resourceName(row.resource_id))} → ${esc(locationName(row.destination_id))}</span><span class="badge ${row.remaining_t>1e-9?'warn':'ok'}">${fmt(row.handoff_count,0)} handoff</span></div><div class="cell-sub">供給元 ${esc(row.source_id?locationName(row.source_id):'未確定')} · ${esc((row.service_ids||[]).map(serviceName).join(' → ')||'経路未確定')}</div><div class="cell-sub">予測到着 ${row.projected_arrival_day==null?'未確定':`Day ${fmt(row.projected_arrival_day,0)}`} · 輸送中 ${fmt(row.pipeline_t,2)} t · 残 ${fmt(row.remaining_t,2)} t</div></div>`).join('')||'<div class="empty-state">このhorizonで広域物流影響はありません。</div>';
    return `${controls}<div class="cell-sub">基準 Day ${fmt(forecast.base_day,0)} → Day ${fmt(forecast.projected_day,0)} · Application filter ${fmt(forecast.period_days,0)}日</div><h3>Future Inventory / steady-state傾向</h3><div class="dependency-card-grid">${inventory}</div><h3>Downstream impact</h3><div class="dependency-card-grid">${impacts}</div><h3>Multi-hop Logistics impact</h3><div class="dependency-card-grid">${logistics}</div>`;
  }
  ...
  function renderInventoryTab(){
    ...
    return `<div class="inventory-decision-surface">
      ...
      <section class="decision-surface-block detailed-forecast-surface"><div class="decision-surface-heading"><div><span class="eyebrow">広域予測</span><h2>Detailed Forecast</h2><p>将来在庫・downstream影響・multi-hop物流・steady-state傾向を、現在/計画依存分析とは別契約で計算します。</p></div></div>${detailedForecastHtml()}</section>
    </div>`;
  }
  ...
  function renderScientificExplorationTab(){
    ...
    return `<div class="exploration-decision-surface"><div class="decision-surface-heading"><div><span class="eyebrow">科学探査</span><h2>科学探査案件</h2><p>候補を選び、右側で実行条件・Fleet拘束・往復移動・RP受入を確認します。輸送とのFleet競合も同じ判断Contextで扱います。</p></div><span class="badge">${items.length} 件</span></div><div class="exploration-card-grid">${cards||'<div class="empty-state">現在利用できる科学探査案件はありません。</div>'}</div></div>`;
  }
  ...
  function renderSurveyTab(){
    ...
    const campaigns=(state.surveys?.campaigns||[]).map((c)=>{
      const provider=c.projected_provider_definition_id?`${c.projected_provider_display_name||'調査Provider'} / ${c.projected_observation_mode_display_name||'—'}`:'未解決';
    ...
    return `<div class="survey-decision-surface"><div class="decision-surface-heading"><div><span class="eyebrow">地表調査</span><h2>地表調査</h2><p>地域、対象資源、調査知識目標を直接選びます。観測手段はApplicationが解決し、戦略差が意思決定に影響する場合だけ比較します。</p></div></div>${createSection}${campaignSection}${knowledgeSection}${providerSection}</div>`;
  }
  ...
  function facilityUpgradeDifferencesHtml(upgrade){
    ...
    if(!rows.length)return '<div class="empty-state">この更新でApplicationが投影する主要な能力差はありません。</div>';
    const value=(raw,unit)=>`${typeof raw==='number'?fmt(raw,2):esc(raw)}${unit?` ${esc(unit)}`:''}`;
    const cards=rows.map((row)=>`<div class="detail-card"><div class="mode-title"><span>${esc(row.label)}</span><span class="badge">${esc(A.userFacingText(row.kind))}</span></div><div class="upgrade-delta"><span><small>Current</small><strong>${value(row.current_value,row.unit)}</strong></span><span aria-hidden="true">→</span><span><small>Target</small><strong>${value(row.target_value,row.unit)}</strong></span></div></div>`).join('');
  ...
  function renderSurveyCampaignInspector(id){
    ...
    const provider=c.projected_provider_definition_id?`${c.projected_provider_display_name||'調査Provider'} @ ${locationName(c.projected_provider_operational_node_id)} / ${c.projected_observation_mode_display_name||'観測方式'}`:'未解決';
  ...
  function renderSurveyCandidateInspector(id){
    ...
    setInspector(`${row.provider_display_name||'調査Provider'} · ${row.observation_mode_display_name||'観測方式'}`,
