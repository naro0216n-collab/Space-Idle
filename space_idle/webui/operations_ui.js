(() => {
  ...
  function constructionPlanControls(prefix,{draftScope=prefix,policyOptions=[],logisticsPolicyOptions=[],selectedPolicy='standard_wait',selectedLogisticsPolicy=null,disabled=false}={}){
    ...
    const logisticsRows=(logisticsPolicyOptions||[]).map((value)=>`<option value="${esc(value)}" ${value===selectedLogisticsPolicy?'selected':''}>${esc(value)}</option>`).join('');
    const disabledAttr=disabled?'disabled':'';
    return `<div class="form-row"><label>優先度<select id="${prefix}PriorityInput" data-draft-key="${esc(draftScope)}:priority" ${disabledAttr}>${priorityOptions(3)}</select></label><label>調達方針<select id="${prefix}ProcurementTimingPolicy" data-draft-key="${esc(draftScope)}:procurement" ${disabledAttr}>${policyRows}</select></label><label>物流Policy<select id="${prefix}LogisticsPolicy" data-draft-key="${esc(draftScope)}:logistics-policy" ${disabledAttr}><option value="" ${selectedLogisticsPolicy==null?'selected':''}>Scenario global / 未割当</option>${logisticsRows}</select></label></div>`;
  }
  ...
  function surfacePlanControls(scope,option,disabled=false){
    ...
    const logistics=(option.logistics_policy_options||[]).map((value)=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');
    const d=disabled?'disabled':'';
    return `<div class="form-row surface-plan-controls"><label>優先度<select data-surface-plan-priority data-draft-key="${esc(scope)}:priority" ${d}>${priorityOptions(3)}</select></label><label>調達方針<select data-surface-plan-policy data-draft-key="${esc(scope)}:procurement" ${d}>${policy}</select></label><label>物流Policy<select data-surface-plan-logistics-policy data-draft-key="${esc(scope)}:logistics-policy" ${d}><option value="">Scenario global / 未割当</option>${logistics}</select></label></div>`;
  }
  const surfacePlanPayload=(button)=>{
    ...
    return {
      ...
      logistics_policy_id:card?.querySelector('[data-surface-plan-logistics-policy]')?.value||null,
    };
  };
  const foundingPlanControls=(scope,option,disabled=false)=>{
    const policies=(option.logistics_policy_options||[]).map((value)=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');
    const d=disabled?'disabled':'';
    return `<div class="form-row"><label>優先度<select data-founding-priority data-draft-key="${esc(scope)}:priority" ${d}>${priorityOptions(3)}</select></label><label>物流Policy<select data-founding-logistics-policy data-draft-key="${esc(scope)}:logistics-policy" ${d}><option value="">Scenario global / 未割当</option>${policies}</select></label></div>`;
  };
  ...
  function renderFacilityInspector(id){
    ...
    if(u){
      ...
      const planControls=constructionPlanControls('upgradePlan',{draftScope:`facility-upgrade:${f.id}`,policyOptions:planOptions.procurement_policy_options||[],logisticsPolicyOptions:planOptions.logistics_policy_options||[],disabled:plan.disabled});
  ...
  function renderProjectInspector(id){
    ...
    const targetRows=[['ID',esc(p.id)],['種別',esc(target)],['状態',esc(stateLabels[p.status]||p.status||(p.paused?'paused':'active'))],['優先度',esc(p.priority??'—')],['施工充足',pct(p.construction_fulfillment??1)],['Projected Material Readiness',esc(readiness)],['調達方針',esc(procurementPolicyName(p.procurement_policy))],['物流Policy',p.resolved_logistics_policy_id?esc(p.resolved_logistics_policy_id):'未解決'],['工数',`${fmt(p.progress??p.construction_done??0)}/${fmt(p.construction_required??0)}`]];
    ...
    const requirementHtml=requirements.length?requirements.map((d)=>{const forecast=d.forecast_requirement_day==null?'指定なし':`Day ${fmt(d.forecast_requirement_day,0)}`;const arrival=d.earliest_confirmed_arrival_day==null?'未確定':`Day ${fmt(d.earliest_confirmed_arrival_day,0)}`;return `<div class="detail-card"><div class="mode-title"><span>${esc(resourceName(d.resource_id))}</span><span>${fmt(d.remaining_t)} t 待ち</span></div><div class="cell-sub">${d.source_mode==='pinned'&&d.allowed_source_ids?.length?esc(locationName(d.allowed_source_ids[0])):d.preferred_source_id?`優先 ${esc(locationName(d.preferred_source_id))}`:d.source_mode==='allow_any'?'Policy委任':d.resolved_policy_id?'Policy指定':'Policy未解決'} → ${esc(locationName(d.destination_id))} · 輸送系内 ${fmt(d.pipeline_t)} t</div><div class="cell-sub">必要時期 ${esc(forecast)} · 最短確定到着 ${esc(arrival)}</div></div>`;}).join(''):'<div class="empty-state">現在のSupply Requirementなし</div>';
    ...
    const logisticsPolicyOptions=(p.logistics_policy_options||[]).map((value)=>`<option value="${esc(value)}" ${value===p.logistics_policy_id?'selected':''}>${esc(value)}</option>`).join('');
    const logisticsPolicyControl=`<div class="form-row"><label>物流Policy<select id="projectLogisticsPolicy" data-draft-key="project:${esc(p.id)}:logistics-policy" ${settingsDisabled}><option value="" ${p.logistics_policy_id==null?'selected':''}>Scenario global / 未割当</option>${logisticsPolicyOptions}</select></label><button type="button" data-set-project-logistics-policy="${esc(p.id)}" data-owner-kind="${foundingProject?'founding':'project'}" ${settingsDisabled}>Policyを適用</button></div>`;
    const projectControls=foundingProject
      ? `<div class="action-stack"><button type="button" data-command="${p.paused?'ResumeFounding':'PauseFounding'}" data-project-id="${esc(p.id)}" ${settingsDisabled}>${p.paused?'準備再開':'準備停止'}</button><div class="form-row"><label>優先度<select id="projectPriorityInput" ${settingsDisabled}>${priorityOptions(p.priority??3)}</select></label><button type="button" data-set-founding-priority="${esc(p.id)}" ${settingsDisabled}>優先度を適用</button></div>${logisticsPolicyControl}<button type="button" class="danger-button" data-command="CancelFounding" data-project-id="${esc(p.id)}" ${settingsDisabled}>Deployment取消</button></div>`
      : `<div class="action-stack"><button type="button" data-command="${p.paused?'ResumeBuild':'PauseBuild'}" data-project-id="${esc(p.id)}" ${settingsDisabled}>${p.paused?'建設再開':'建設停止'}</button><div class="form-row"><label>優先度<select id="projectPriorityInput" data-draft-key="project:${esc(p.id)}:priority" ${settingsDisabled}>${priorityOptions(p.priority??3)}</select></label><button type="button" data-set-project-priority="${esc(p.id)}" ${settingsDisabled}>優先度を適用</button></div><div class="form-row"><label>調達方針<select id="projectProcurementTimingPolicy" data-draft-key="project:${esc(p.id)}:procurement" ${procurementDisabled}>${procurementOptions}</select></label><button type="button" data-set-project-procurement="${esc(p.id)}" ${procurementDisabled}>方針を適用</button></div>${logisticsPolicyControl}<button type="button" class="danger-button" data-command="CancelBuild" data-project-id="${esc(p.id)}" ${settingsDisabled}>案件取消</button></div>`;
  ...
  function renderBuildOptionInspector(id){
    ...
    const planOptions=state.buildOptions||{};
    const planControls=constructionPlanControls('buildPlan',{draftScope:`facility-build:${o.facility_definition_id}`,policyOptions:planOptions.procurement_policy_options||[],logisticsPolicyOptions:planOptions.logistics_policy_options||[],disabled:plan.disabled});
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const surfaceFound=event.target.closest('[data-surface-found]');if(surfaceFound){const card=surfaceFound.closest('.surface-action-card'),displayName=card?.querySelector('[data-new-location-name]')?.value.trim();if(!displayName){banner('Location名を入力してください','error');return;}const priority=Number(card?.querySelector('[data-founding-priority]')?.value??3),logisticsPolicyId=card?.querySelector('[data-founding-logistics-policy]')?.value||null;try{await command('PlanOperationalNodeFounding',{staging_node_id:surfaceFound.dataset.stagingNodeId,display_name:displayName,target_spec:{target_type:'surface_location',body_id:surfaceFound.dataset.bodyId,core_cell_id:surfaceFound.dataset.cellId},deployment_recipe_id:surfaceFound.dataset.recipeId,vehicle_definition_id:surfaceFound.dataset.vehicleId,priority,logistics_policy_id:logisticsPolicyId});banner('Founding Deploymentを開始しました');}catch{}return;}
    const build=event.target.closest('[data-build]');if(build){const prefix=build.dataset.planPrefix||'buildPlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),procurementPolicy=$(`#${prefix}ProcurementTimingPolicy`)?.value||'standard_wait',logisticsPolicyId=$(`#${prefix}LogisticsPolicy`)?.value||null;try{await command('PlanBuild',{operational_node_id:state.operationalNodeId,facility_id:build.dataset.build,priority,procurement_policy:procurementPolicy,logistics_policy_id:logisticsPolicyId});banner('建設計画を作成しました');}catch{}return;}
    const upgrade=event.target.closest('[data-upgrade]');if(upgrade){const prefix=upgrade.dataset.planPrefix||'upgradePlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),procurementPolicy=$(`#${prefix}ProcurementTimingPolicy`)?.value||'standard_wait',logisticsPolicyId=$(`#${prefix}LogisticsPolicy`)?.value||null;try{const result=await command('PlanFacilityUpgrade',{facility_id:upgrade.dataset.upgrade,priority,procurement_policy:procurementPolicy,logistics_policy_id:logisticsPolicyId});banner(`Upgrade案件 ${result?.created_id||''} を作成しました`);}catch{}return;}
    ...
    const projectPolicy=event.target.closest('[data-set-project-logistics-policy]');if(projectPolicy){const policyId=$('#projectLogisticsPolicy').value||null,ownerKind=projectPolicy.dataset.ownerKind||'project',ownerId=projectPolicy.dataset.setProjectLogisticsPolicy;try{if(policyId)await command('AssignLogisticsPolicy',{owner_kind:ownerKind,owner_id:ownerId,policy_id:policyId});else await command('UnassignLogisticsPolicy',{owner_kind:ownerKind,owner_id:ownerId});}catch{}return;}
