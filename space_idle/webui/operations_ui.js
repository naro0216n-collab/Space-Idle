(() => {
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const upgrade=event.target.closest('[data-upgrade]');if(upgrade){const prefix=upgrade.dataset.planPrefix||'upgradePlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),procurementPolicy=$(`#${prefix}ProcurementTimingPolicy`)?.value||'standard_wait';try{await command('PlanFacilityUpgrade',{facility_id:upgrade.dataset.upgrade,priority,procurement_policy:procurementPolicy});banner('設備更新案件を作成しました');}catch{}return;}
    const decommission=event.target.closest('[data-decommission]');if(decommission){const prefix=decommission.dataset.planPrefix||'decommissionPlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),procurementPolicy=$(`#${prefix}ProcurementTimingPolicy`)?.value||'standard_wait';try{await command('PlanFacilityDecommission',{facility_id:decommission.dataset.decommission,priority,procurement_policy:procurementPolicy});state.inspector={type:'facility',id:decommission.dataset.decommission};renderInspector();banner('設備撤去案件を作成しました');}catch{}return;}
