(() => {
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const surfaceFound=event.target.closest('[data-surface-found]');if(surfaceFound){const card=surfaceFound.closest('.surface-action-card'),displayName=card?.querySelector('[data-new-location-name]')?.value.trim();if(!displayName){banner('Location名を入力してください','error');return;}const priority=Number(card?.querySelector('[data-founding-priority]')?.value??3),logisticsPolicyId=card?.querySelector('[data-founding-logistics-policy]')?.value||null;try{await command('FoundLocation',{staging_node_id:surfaceFound.dataset.stagingNodeId,display_name:displayName,body_id:surfaceFound.dataset.bodyId,core_cell_id:surfaceFound.dataset.cellId,founding_package_id:surfaceFound.dataset.packageId,vehicle_definition_id:surfaceFound.dataset.vehicleId,priority,logistics_policy_id:logisticsPolicyId});banner('Founding Deploymentを開始しました');}catch{}return;}
