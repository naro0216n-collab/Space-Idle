(() => {
  ...
  function renderSurveyTab(){
    ...
    const providerSection=`<section class="card"><div class="card-heading"><h3>Survey Provider Fleet</h3><span class="badge">${assignedCount}</span></div><div class="card-body"><div class="cell-sub">Fleet-backed Survey Provider用途へ希望Fleet数量を設定します。Campaignの停止・再開とは独立した持続状態です。</div><div class="detail-stack">${providerCards||'<div class="empty-state">Fleet-backed Survey Providerなし</div>'}</div></div></section>`;
  ...
  function renderSurveyInspector(id){
    ...
    const assignment=(state.surveys?.provider_fleet||[]).find((row)=>Number(row.committed_units||0)>0&&row.provider_definition_id===s.provider_definition_id&&row.operational_node_id===s.provider_operational_node_id);
