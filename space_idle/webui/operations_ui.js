(() => {
  ...
  function constructionPlanControls(prefix,{draftScope=prefix,policyOptions=[],selectedPolicy='standard_wait',disabled=false}={}){
    ...
    return `<div class="form-row">${priorityControl(3,`id="${prefix}PriorityInput" data-draft-key="${esc(draftScope)}:priority"`,'優先度',disabled)}<label>調達方針<select id="${prefix}ProcurementTimingPolicy" data-draft-key="${esc(draftScope)}:procurement" ${disabledAttr}>${policyRows}</select></label></div>`;
  }
  ...
  function surfacePlanControls(scope,option,disabled=false){
    ...
    return `<div class="form-row surface-plan-controls">${priorityControl(3,`data-surface-plan-priority data-draft-key="${esc(scope)}:priority"`,'優先度',disabled)}<label>調達方針<select data-surface-plan-policy data-draft-key="${esc(scope)}:procurement" ${d}>${policy}</select></label></div>`;
  }
  ...
  const foundingPlanControls=(scope,option,disabled=false)=>{
    const d=disabled?'disabled':'';
    return `<div class="form-row">${priorityControl(3,`data-founding-priority data-draft-key="${esc(scope)}:priority"`,'優先度',disabled)}</div>`;
  };
  ...
  function renderSurveyTab(){
    ...
    if(cells.length){
      ...
      const nodes=cells.map((cell)=>{const layers=surveyLayerSummary(cell,knowledgeByCell.get(cell.id)||[]);return `<label class="survey-scope-node" style="left:${pos[cell.id].x}%;top:${pos[cell.id].y}%"><input type="checkbox" data-survey-draft-cell data-draft-key="survey:new:cell:${esc(cell.id)}" value="${esc(cell.id)}"><span><strong>${esc(cellMap.get(cell.id)||cell.display_name||cell.id)}</strong>${Object.entries(layers).map(([layer,value])=>`<small data-survey-layer-value="${esc(layer)}">${esc(value)}</small>`).join('')}</span></label>`;}).join('');
  ...
  function renderFacilityInspector(id){
    ...
    if(industry){
      ...
      const processControl=`<div class="form-row"><label>生産工程<select id="facilityProcessSelect" data-draft-key="facility:${esc(f.id)}:process" ${processOptions?'':'disabled'}>${processOptions||'<option>候補なし</option>'}</select></label><button type="button" data-set-facility-process="${esc(f.id)}" ${processOptions?'':'disabled'}>工程を適用</button></div>`;
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const upgrade=event.target.closest('[data-upgrade]');if(upgrade){const prefix=upgrade.dataset.planPrefix||'upgradePlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),procurementPolicy=$(`#${prefix}ProcurementTimingPolicy`)?.value||'standard_wait';try{const result=await command('PlanFacilityUpgrade',{facility_id:upgrade.dataset.upgrade,priority,procurement_policy:procurementPolicy});banner('設備更新案件を作成しました');}catch{}return;}
    ...
    const process=event.target.closest('[data-set-facility-process]');if(process){const select=$('#facilityProcessSelect');if(select?.value){try{await command('SetFacilityProcess',{facility_id:process.dataset.setFacilityProcess,process_id:select.value});banner('生産工程を更新しました');}catch{}}return;}
