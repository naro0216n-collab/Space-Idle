(() => {
  ...
  function renderFacilitiesTab(){
    const rows=(state.operationalNode?.facilities||[]).map((f)=>{const u=f.next_upgrade;const upgrade=u?(u.active_project_id?`案件 ${esc(u.active_project_id)}`:`→ Lv ${fmt(u.target_level,0)}`):'—';return `<tr class="selectable ${state.inspector?.type==='facility'&&state.inspector.id===f.id?'is-selected':''}" data-inspect="facility" data-id="${esc(f.id)}"><td><div class="cell-main">${esc(f.display_name)}</div><div class="cell-sub">${esc(f.id)}</div></td><td>Lv ${fmt(f.level,0)}</td><td>${f.paused?'<span class="badge warn">停止</span>':'<span class="badge ok">稼働</span>'}</td><td>${pct(f.operational_utilization)}</td><td>${pct(f.maintenance_satisfaction)}</td><td>${esc(priorityName(f.activity_priority))}</td><td>${esc(priorityName(f.maintenance_priority??3))}</td><td>${upgrade}</td><td>${(f.activation_blockers||[]).length}</td></tr>`;}).join('');
  ...
  function renderSurveyCampaignInspector(id){
    ...
    setInspector(`Survey Campaign ${c.id}`,
      ...
      section('固定条件',`<div class="cell-sub">Provider: ${esc(c.provider_constraint_definition_id?`${definitionName(c.provider_constraint_definition_id)} @ ${locationName(c.provider_constraint_operational_node_id)}`:'auto')} · Mode: ${esc(c.observation_mode_constraint||'auto')}</div><button type="button" data-clear-survey-constraint="${esc(c.id)}">固定条件を解除</button>`)+
