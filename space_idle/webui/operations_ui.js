(() => {
  ...
  function constructionPlanControls(prefix,{draftScope=prefix,policyOptions=[],logisticsPolicyOptions=[],selectedPolicy='mixed',selectedLogisticsPolicy=null,disabled=false}={}){
    ...
    return `<div class="form-row"><label>優先度<select id="${prefix}PriorityInput" data-draft-key="${esc(draftScope)}:priority" ${disabledAttr}>${priorityOptions(3)}</select></label><label>調達方針<select id="${prefix}SourcingPolicy" data-draft-key="${esc(draftScope)}:sourcing" ${disabledAttr}>${policyRows}</select></label><label>物流Policy<select id="${prefix}LogisticsPolicy" data-draft-key="${esc(draftScope)}:logistics-policy" ${disabledAttr}><option value="" ${selectedLogisticsPolicy==null?'selected':''}>Scenario global / 未割当</option>${logisticsRows}</select></label></div>`;
  }
  ...
  function surfacePlanControls(scope,option,disabled=false){
    ...
    return `<div class="form-row surface-plan-controls"><label>優先度<select data-surface-plan-priority data-draft-key="${esc(scope)}:priority" ${d}>${priorityOptions(3)}</select></label><label>調達方針<select data-surface-plan-policy data-draft-key="${esc(scope)}:sourcing" ${d}>${policy}</select></label><label>物流Policy<select data-surface-plan-logistics-policy data-draft-key="${esc(scope)}:logistics-policy" ${d}><option value="">Scenario global / 未割当</option>${logistics}</select></label></div>`;
  }
  ...
  function renderFacilityInspector(id){
    ...
    if(u){
      ...
      const planControls=constructionPlanControls('upgradePlan',{draftScope:`facility-upgrade:${f.id}`,policyOptions:planOptions.sourcing_policy_options||[],logisticsPolicyOptions:planOptions.logistics_policy_options||[],disabled:plan.disabled});
  ...
  function renderProjectInspector(id){
    ...
    const targetRows=[['ID',esc(p.id)],['種別',esc(target)],['状態',esc(stateLabels[p.status]||p.status||(p.paused?'paused':'active'))],['優先度',esc(p.priority??'—')],['施工充足',pct(p.construction_fulfillment??1)],['Projected Material Readiness',esc(readiness)],['調達方針',esc(sourcingPolicyName(p.sourcing_policy))],['物流Policy',p.resolved_logistics_policy_id?esc(p.resolved_logistics_policy_id):'未解決'],['工数',`${fmt(p.progress??p.construction_done??0)}/${fmt(p.construction_required??0)}`]];
  ...
  function renderBuildOptionInspector(id){
    ...
    const planOptions=state.buildOptions||{};
    const planControls=constructionPlanControls('buildPlan',{draftScope:`facility-build:${o.facility_definition_id}`,policyOptions:planOptions.sourcing_policy_options||[],logisticsPolicyOptions:planOptions.logistics_policy_options||[],disabled:plan.disabled});
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const build=event.target.closest('[data-build]');if(build){const prefix=build.dataset.planPrefix||'buildPlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),sourcingPolicy=$(`#${prefix}SourcingPolicy`)?.value||'mixed',logisticsPolicyId=$(`#${prefix}LogisticsPolicy`)?.value||null;try{await command('PlanBuild',{operational_node_id:state.operationalNodeId,facility_id:build.dataset.build,priority,sourcing_policy:sourcingPolicy,logistics_policy_id:logisticsPolicyId});banner('建設計画を作成しました');}catch{}return;}
    const upgrade=event.target.closest('[data-upgrade]');if(upgrade){const prefix=upgrade.dataset.planPrefix||'upgradePlan',priority=Number($(`#${prefix}PriorityInput`)?.value??3),sourcingPolicy=$(`#${prefix}SourcingPolicy`)?.value||'mixed',logisticsPolicyId=$(`#${prefix}LogisticsPolicy`)?.value||null;try{const result=await command('PlanFacilityUpgrade',{facility_id:upgrade.dataset.upgrade,priority,sourcing_policy:sourcingPolicy,logistics_policy_id:logisticsPolicyId});banner(`Upgrade案件 ${result?.created_id||''} を作成しました`);}catch{}return;}
