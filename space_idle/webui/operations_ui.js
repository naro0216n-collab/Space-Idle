(() => {
  ...
  function renderProjectInspector(id){
    ...
    if(p.target_kind==='location_founding_deployment'){
      const targetRows=[['ID',esc(p.id)],['種別',esc(target)],['状態',esc(stateLabels[p.status]||p.status)],['Staging',esc(locationName(p.location_id))],['対象Cell',esc(surfaceCellLabel(p.target_cell_id))],['生成予定Location',esc(p.target_location_id||'—')],['Vehicle',esc(definitionName(p.founding_vehicle_definition_id))],['準備作業',`${fmt(p.preparation_done??0,2)}/${fmt(p.preparation_required??0,2)}`],['Deployment',`${fmt(p.deployment_progress_days??0,2)}/${fmt(p.deployment_required_days??0,2)} 日`],['優先度',esc(p.priority??'—')]];
      ...
      const demands=(state.demands||[]).filter((d)=>d.owner_kind==='location_founding'&&d.owner_id===p.id);
      ...
      const closed=p.status==='complete'||p.status==='cancelled';
      const priorityEditable=p.status==='preparing';
      const canCancel=p.status==='preparing'&&!p.materials_committed;
      const controls=`<div class="action-stack"><button type="button" data-command="${p.paused?'ResumeFounding':'PauseFounding'}" data-project-id="${esc(p.id)}" ${closed?'disabled':''}>${p.paused?'Deployment再開':'Deployment停止'}</button><div class="form-row"><label>優先度<input id="foundingPriorityInput" type="number" step="1" value="${esc(p.priority??50)}" data-draft-key="founding:${esc(p.id)}:priority" ${priorityEditable?'':'disabled'}></label><button type="button" data-set-founding-priority="${esc(p.id)}" ${priorityEditable?'':'disabled'}>優先度を適用</button></div><button type="button" class="danger-button" data-command="CancelFounding" data-project-id="${esc(p.id)}" ${canCancel?'':'disabled'}>Deployment取消</button></div>`;
      setInspector(p.display_name||p.id,section('Founding Deployment',kv(targetRows))+section('必要資源 / Staging',resourceRows||'<div class="empty-state">追加資源なし</div>')+section('物流Demand',demandHtml)+section('Blocker',(p.blockers||[]).length?`<div class="issue-stack">${p.blockers.map(issueHtml).join('')}</div>`:'<span class="badge ok">なし</span>')+section('操作',controls));
      return true;
    }
    const targetRows=[['ID',esc(p.id)],['種別',esc(target)],['状態',esc(stateLabels[p.status]||p.status||(p.paused?'paused':'active'))],['優先度',esc(p.priority??'—')],['施工配分',esc(fmt(p.construction_weight??0,2))],['施工充足',pct(p.construction_fulfillment??1)],['調達方針',esc(sourcingPolicyName(p.sourcing_policy))],['優先供給元',p.import_source_id?esc(locationName(p.import_source_id)):'物流Laneから自動選択'],['工数',`${fmt(p.progress??p.construction_done??0)}/${fmt(p.construction_required??0)}`]];
  ...
  function renderSurfaceCellInspector(id){
    ...
    const foundation=(cell.foundation_options||[]).map((option)=>{
      ...
      const scope=`foundation:${cell.id}:${option.package_id}:${option.staging_location_id}:${option.vehicle_definition_id}`;
      return `<div class="route-mode-card surface-action-card"><div class="mode-title"><span>${esc(option.package_name)} · ${esc(option.staging_location_name)}</span><span class="badge ${disabled?'warn':'ok'}">${active?'案件進行中':blockers.length?`${blockers.length} blocker`:'設立可'}</span></div><div class="cell-sub">輸送 ${fmt(option.deployment_days,0)} 日 · Payload ${fmt(option.payload_t,2)} t · Vehicle ${esc(option.vehicle_name)}</div><div class="surface-resource-list">${tupleResourcesHtml(option.resources)}</div>${blockers.length?`<div class="issue-stack">${blockers.map(issueHtml).join('')}</div>`:''}${active?`<div class="cell-sub">Active Project: ${esc(active)}</div>`:''}<div class="form-row"><label>Location名<input type="text" data-new-location-name data-draft-key="${esc(scope)}:name" placeholder="新規地表拠点" ${disabled?'disabled':''}></label><label>優先度<input type="number" step="1" value="50" data-founding-priority data-draft-key="${esc(scope)}:priority" ${disabled?'disabled':''}></label></div><button type="button" class="primary" data-surface-found="1" data-package-id="${esc(option.package_id)}" data-staging-location-id="${esc(option.staging_location_id)}" data-vehicle-definition-id="${esc(option.vehicle_definition_id)}" data-cell-id="${esc(cell.id)}" ${disabled?'disabled':''}>このCellへFounding Deploymentを開始</button></div>`;
    }).join('')||'<div class="empty-state">このCellへ利用できるFounding Deployment候補がありません。</div>';
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const surfaceFound=event.target.closest('[data-surface-found]');if(surfaceFound){const card=surfaceFound.closest('.surface-action-card'),displayName=card?.querySelector('[data-new-location-name]')?.value.trim();if(!displayName){banner('Location名を入力してください','error');return;}const priority=Number(card?.querySelector('[data-founding-priority]')?.value??50);try{await command('FoundLocation',{package_id:surfaceFound.dataset.packageId,staging_location_id:surfaceFound.dataset.stagingLocationId,core_cell_id:surfaceFound.dataset.cellId,display_name:displayName,vehicle_definition_id:surfaceFound.dataset.vehicleDefinitionId,priority});banner('Founding Deploymentを開始しました');}catch{}return;}
    ...
    const foundingPriority=event.target.closest('[data-set-founding-priority]');if(foundingPriority){try{await command('SetFoundingPriority',{project_id:foundingPriority.dataset.setFoundingPriority,priority:Number($('#foundingPriorityInput').value)});}catch{}return;}
