(() => {
  ...
  function renderSurveyTab(){
    ...
    const providerCards=providerFleet.map((row)=>`<div class="detail-card" data-survey-provider-fleet-card><div class="mode-title"><span>${esc(definitionName(row.provider_definition_id))}</span><span>assigned ${fmt(row.committed_units,0)} · free ${fmt(row.free_units,0)}</span></div><div class="cell-sub">${esc(locationName(row.operational_node_id))} · ${esc(definitionName(row.vehicle_definition_id))} · Survey capacity ${fmt(row.capacity_units_per_day,2)}/日</div>${row.fleet_commitment_id?`<div class="cell-sub">Fleet commitment ${esc(row.fleet_commitment_id)}</div>`:''}${(row.blockers||[]).length?`<div class="issue-stack">${row.blockers.map((b)=>issueHtml(['survey_provider',b])).join('')}</div>`:''}<div class="form-row"><label>Fleet unit<input type="number" min="0" max="${Math.max(0,Number(row.max_units||0))}" step="1" value="${fmt(row.committed_units,0)}" data-survey-provider-fleet-quantity data-draft-key="survey-provider:${esc(row.provider_definition_id)}:${esc(row.operational_node_id)}:quantity" ${row.can_set_quantity?'':'disabled'}></label><button type="button" data-survey-provider-set-fleet="${esc(row.provider_definition_id)}" data-vehicle-definition-id="${esc(row.vehicle_definition_id)}" data-operational-node-id="${esc(row.operational_node_id)}" ${row.can_set_quantity?'':'disabled'}>Fleet数量を適用</button></div></div>`).join('');
    const assignedCount=providerFleet.filter((row)=>Number(row.committed_units||0)>0).length;
    const providerSection=`<section class="card"><div class="card-heading"><h3>Survey Provider Fleet</h3><span class="badge">${assignedCount}</span></div><div class="card-body"><div class="cell-sub">Fleet-backed Survey Provider用途へ希望Fleet数量を設定します。CampaignのPauseとは独立した持続状態です。</div><div class="detail-stack">${providerCards||'<div class="empty-state">Fleet-backed Survey Providerなし</div>'}</div></div></section>`;
    ...
    const campaigns=(state.surveys?.campaigns||[]).map((c)=>{
      ...
      const fleet=c.required_fleet_units==null?'—':`${fmt(c.assigned_fleet_units,0)} / req ${fmt(c.required_fleet_units,0)}`;
      const eta=c.projected_remaining_days==null?'—':`${fmt(c.projected_remaining_days,1)}日`;
      return `<tr class="selectable" data-inspect="survey-campaign" data-id="${esc(c.id)}"><td><div class="cell-main">${esc(c.id)}</div><div class="cell-sub">${c.target_cell_ids.length} Cell × ${c.resource_ids.length} Resource</div></td><td>${esc(stateLabels[c.status]||c.status)}</td><td>K${fmt(c.goal_knowledge_level,0)} · ${fmt(c.covered_targets,0)}/${fmt(c.covered_targets+c.remaining_targets,0)}</td><td>${esc(provider)}</td><td>${esc(fleet)}</td><td>${fmt(c.capacity_points_per_day,2)}</td><td>${esc(eta)}</td><td>${(c.blockers||[]).length}</td></tr>`;
    }).join('');
    const campaignSection=`<section class="card"><div class="card-heading"><h3>Survey Campaign</h3><span class="badge">${(state.surveys?.campaigns||[]).length}</span></div><div class="table-wrap"><table><thead><tr><th>Campaign</th><th>状態</th><th>Goal / covered</th><th>Projected Provider / Mode</th><th>Fleet</th><th>Capacity/日</th><th>予測残日数</th><th>blocker</th></tr></thead><tbody>${campaigns||'<tr><td colspan="8">Campaignなし</td></tr>'}</tbody></table></div></section>`;

    const rows=visibleKnowledge.map((s)=>{const potential=s.visible_potential==null?'—':fmt(s.visible_potential,3),precision=s.visible_potential_precision_fraction==null?'':s.visible_potential_precision_fraction<=0?' · 測定済み':` · 精度 ±${pct(s.visible_potential_precision_fraction)}`;return `<tr class="selectable" data-inspect="survey" data-id="${esc(`${s.cell_id}::${s.resource_id}`)}"><td><div class="cell-main">${esc(s.resource_name)}</div><div class="cell-sub">${esc(s.cell_label)}</div></td><td>Knowledge ${fmt(s.knowledge_level,0)}</td><td>${fmt(s.progress,2)}</td><td>${s.presence_probability==null?'—':pct(s.presence_probability)}</td><td>${potential}${precision}</td></tr>`;}).join('');
    const knowledgeSection=`<section class="card"><div class="card-heading"><h3>Survey Knowledge</h3></div><div class="table-wrap"><table><thead><tr><th>資源・Cell</th><th>Knowledge</th><th>Progress</th><th>存在確率</th><th>Resource Potential</th></tr></thead><tbody>${rows||'<tr><td colspan="5">この天体にSurvey対象なし</td></tr>'}</tbody></table></div></section>`;
    return `${providerSection}${createSection}${campaignSection}${knowledgeSection}`;
  }
  ...
  function renderFacilityInspector(id){
    ...
    const salvageSection=section('Decommission Recovery',kv([['Recovery potential',esc(salvagePotential)],['Projected recoverable fraction',pct(f.projected_salvage_fraction??1)],['Projected recovery',esc(salvageProjected)],['Existing-stock blocker',esc((f.decommission_blockers||[]).map(([code,detail])=>`${code}: ${detail}`).join(' / ')||'なし')]]));
  ...
  function renderSurveyInspector(id){
    ...
    setInspector(`${s.resource_name} · ${s.cell_label}`,section('Knowledge State',kv([['Knowledge Level',String(s.knowledge_level)],['Progress',fmt(s.progress,2)],['存在確率',s.presence_probability==null?'—':pct(s.presence_probability)],['Resource Potential',potential],['推定精度',precision]]))+section('Campaign',containing.length?containing.map((c)=>`<button type="button" data-inspect="survey-campaign" data-id="${esc(c.id)}">${esc(c.id)} · goal K${fmt(c.goal_knowledge_level,0)}</button>`).join(' '):'<div class="cell-sub">このtargetを含むCampaignなし。Surveyタブでscopeをまとめて作成します。</div>'));return true;
  }
  ...
  function renderSurveyCampaignInspector(id){
    ...
    const cellChecks=[...cellMap].map(([cellId,label])=>`<label class="check-row"><input type="checkbox" data-survey-campaign-cell value="${esc(cellId)}" ${(c.target_cell_ids||[]).includes(cellId)?'checked':''}>${esc(label)}</label>`).join('');
    const resourceChecks=[...resourceMap].map(([resourceId,label])=>`<label class="check-row"><input type="checkbox" data-survey-campaign-resource value="${esc(resourceId)}" ${(c.resource_ids||[]).includes(resourceId)?'checked':''}>${esc(label)}</label>`).join('');
    const candidates=(c.candidates||[]).map((row)=>`<div class="detail-card"><div class="mode-title"><span>${esc(definitionName(row.provider_definition_id))} / ${esc(row.observation_mode_id)}</span><span class="badge ${row.viable?'ok':'warn'}">${row.viable?'eligible':'blocked'}</span></div><div class="cell-sub">${esc(locationName(row.provider_operational_node_id))} · ${esc(row.provider_source_kind)} · rate ${fmt(row.survey_rate,2)} · Kmax ${fmt(row.max_knowledge_level,0)} · source ${fmt(row.assigned_source_units,0)} / min ${fmt(row.minimum_source_units,0)} · capacity ${fmt(row.capacity_units_per_day,2)}</div><div class="cell-sub">precision est ±${pct(row.estimate_uncertainty_fraction)} / measured ±${pct(row.measurement_precision_fraction)}</div>${(row.blockers||[]).length?`<div class="issue-stack">${row.blockers.map((b)=>issueHtml(['survey',b])).join('')}</div>`:''}<button type="button" data-survey-constrain-candidate="${esc(c.id)}" data-provider-definition-id="${esc(row.provider_definition_id)}" data-operational-node-id="${esc(row.provider_operational_node_id)}" data-observation-mode-id="${esc(row.observation_mode_id)}">このProvider / Modeをhard constraintに設定</button></div>`).join('')||'<div class="empty-state">現在利用可能なProvider context候補なし</div>';
    ...
    const blockers=c.blockers||[],provider=c.projected_provider_definition_id?`${definitionName(c.projected_provider_definition_id)} @ ${locationName(c.projected_provider_operational_node_id)} / ${c.projected_observation_mode_id}`:'未解決';
    ...
    setInspector(`Survey Campaign ${c.id}`,
      ...
      section('操作',`<div class="action-stack">${actions}<div class="form-row"><label>活動優先度<select id="surveyPriorityInput">${priorityOptions(c.priority??3)}</select></label><button data-set-survey-priority="${esc(c.id)}" ${c.can_set_priority?'':'disabled'}>優先度を適用</button></div></div>`)
    );return true;
  }
