(() => {
  ...
  function surveyComparisonHtml(campaign){
    return comparisonSurfaceHtml({
      ...
      headingHtml:(row)=>`<strong>${esc(definitionName(row.provider_definition_id))}</strong><span>${esc(row.observation_mode_display_name||'観測方式')}</span><small>${esc(locationName(row.provider_operational_node_id))}</small>`,
  ...
  function renderSurveyTab(){
    ...
    const campaigns=(state.surveys?.campaigns||[]).map((c)=>{
      const provider=c.projected_provider_definition_id?`${definitionName(c.projected_provider_definition_id)} / ${c.projected_observation_mode_display_name||'—'}`:'未解決';
  ...
  function renderSurveyCampaignInspector(id){
    ...
    const provider=c.projected_provider_definition_id?`${definitionName(c.projected_provider_definition_id)} @ ${locationName(c.projected_provider_operational_node_id)} / ${c.projected_observation_mode_display_name||'観測方式'}`:'未解決';
  ...
  function renderSurveyCandidateInspector(id){
    ...
    setInspector(`${definitionName(row.provider_definition_id)} · ${row.observation_mode_display_name||'観測方式'}`,
