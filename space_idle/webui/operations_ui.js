(() => {
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const surfaceFound=event.target.closest('[data-surface-found]');if(surfaceFound){const card=surfaceFound.closest('.surface-action-card'),displayName=card?.querySelector('[data-new-location-name]')?.value.trim();if(!displayName){banner('Location名を入力してください','error');return;}const plan=surfacePlanPayload(surfaceFound);try{await command('FoundLocation',{provider_location_id:surfaceFound.dataset.surfaceFound,display_name:displayName,body_id:surfaceFound.dataset.bodyId,core_cell_id:surfaceFound.dataset.cellId,...plan});banner('Location設立Projectを作成しました');}catch{}return;}
