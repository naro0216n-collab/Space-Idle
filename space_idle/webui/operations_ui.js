(() => {
  ...
  function constructionPlanControls(prefix,{policyOptions=[],sourceOptions=[],selectedPolicy='mixed',selectedSource=null,disabled=false}={}){
    ...
    return `<div class="form-row"><label>優先度<input id="${prefix}PriorityInput" type="number" step="1" value="50" ${disabledAttr}></label><label>調達方針<select id="${prefix}SourcingPolicy" ${disabledAttr}>${policyRows}</select></label><label>優先供給元<select id="${prefix}ImportSource" ${disabledAttr}><option value="" ${selectedSource==null?'selected':''}>物流Laneから自動選択</option>${sourceRows}</select></label></div>`;
  }
  ...
  function renderFacilityInspector(id){
    ...
    if(u){
      ...
      const planControls=constructionPlanControls('upgradePlan',{policyOptions:planOptions.sourcing_policy_options||[],sourceOptions:planOptions.import_source_options||[],disabled:Boolean(blocked)});
  ...
  function renderBuildOptionInspector(id){
    ...
    const planOptions=state.buildOptions||{};
    const planControls=constructionPlanControls('buildPlan',{policyOptions:planOptions.sourcing_policy_options||[],sourceOptions:planOptions.import_source_options||[],disabled:Boolean(blockers.length)});
