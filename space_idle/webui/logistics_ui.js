(() => {
  ...
  function renderMarket(){
    ...
    const createCard=`<section class="market-create-card" data-new-market-order><div class="decision-surface-heading"><div><span class="eyebrow">新規注文</span><h3>取引を設定</h3><p>市場・方向・資源を選び、総量または日量のどちらか一方を目標として設定します。</p></div></div><div class="market-create-grid"><label>市場<select data-market-interface data-draft-key="market:new:interface" data-structured-draft data-draft-scope="market:new">${marketInterfaceOptions()}</select></label><label>方向<select data-market-direction data-draft-key="market:new:direction" data-structured-draft data-draft-scope="market:new"><option value="buy">買う</option><option value="sell">売る</option></select></label><label>資源<select data-market-resource data-draft-key="market:new:resource" data-structured-draft data-draft-scope="market:new">${marketResourceOptions()}</select></label><label>取引目標${marketTargetFields('market:new','quantity',0,null)}</label>${priorityControl(3,'data-market-priority data-draft-key="market:new:priority" data-structured-draft data-draft-scope="market:new"')}<label>価格条件<input data-market-price type="number" min="0" step="0.01" placeholder="買い上限 / 売り下限" data-draft-key="market:new:price" data-structured-draft data-draft-scope="market:new"></label></div><button type="button" class="primary" data-market-create ${canCreate?'':'disabled'}>取引注文を作成</button></section>`;
  ...
  document.addEventListener('click',async(event)=>{
    ...
    const marketCreate=event.target.closest('[data-market-create]');if(marketCreate){const root=marketCreate.closest('[data-new-market-order]');try{await command('CreateTradeOrder',marketOrderPayload(root,{create:true}));A.completeActiveDraft('market:new');}catch{}return;}
    const marketSave=event.target.closest('[data-market-save]');if(marketSave){const root=marketSave.closest('[data-market-order-row]');try{await command('UpdateTradeOrder',{order_id:marketSave.dataset.marketSave,...marketOrderPayload(root)});A.completeActiveDraft(`market:${marketSave.dataset.marketSave}`);}catch{}return;}
