(() => {
  ...
  let editingLogisticsPolicyId=null;
  ...
  const policyLabels={balanced:'時間・推進剤バランス',fastest:'最速',lowest_propellant:'推進剤最少'};
  ...
  function renderSupplyPolicies(){
    const policies=logistics().logistics_policies||[],targets=logistics().target_stocks||[];
    $('#logisticsPolicyCountBadge').textContent=`${policies.length}方針 / ${targets.length}目標`;
    const policyRows=policies.map((row)=>{const allowed=(row.allowed_source_ids||[]).map(locationName),preferred=row.preferred_source_id?locationName(row.preferred_source_id):null;const sourceText=row.source_mode==='pinned'?`固定: ${allowed[0]||'—'}`:row.source_mode==='preferred'?`優先: ${preferred||'—'}${allowed.length?` / 許可 ${allowed.join(', ')}`:''}`:`許可候補から自動選択${allowed.length?`: ${allowed.join(', ')}`:''}`;const pathText=row.path_mode==='pinned'?`固定: ${(row.explicit_path||[]).map(definitionName).join(' → ')}`:row.path_mode==='preferred'?`Preference: ${policyLabel(row.path_preference)}`:'標準BALANCED';const owners=(row.assigned_owners||[]).map(([kind,id])=>`${ownerLabel(kind)}:${id}`).join(' / ')||'個別割当なし';return `<tr><td><div class="cell-main">${esc(row.id)} ${row.is_global?'<span class="badge ok">GLOBAL</span>':''}</div><div class="cell-sub">${esc(owners)}</div></td><td>${esc(sourceText)}</td><td>${esc(pathText)}</td><td><div class="action-row"><button type="button" data-logistics-policy-edit="${esc(row.id)}">編集</button>${row.is_global?'<button type="button" data-logistics-global-clear>Global解除</button>':`<button type="button" data-logistics-global-set="${esc(row.id)}">Global</button>`}<button type="button" class="danger-button" data-logistics-policy-delete="${esc(row.id)}">削除</button></div></td></tr>`;}).join('');
    ...
    $('#logisticsPolicyTable').innerHTML=`<div style="padding:8px" class="action-row"><button type="button" class="primary" id="newLogisticsPolicyButton">Logistics Policyを作成</button><button type="button" id="newTargetStockButton">Target Stockを設定</button></div><div class="cell-main" style="padding:8px">Logistics Policy</div><table><thead><tr><th>Policy / 割当</th><th>供給元</th><th>経路</th><th>操作</th></tr></thead><tbody>${policyRows||'<tr><td colspan="4">Logistics Policyなし。複数供給元があるSupply Requirementはsource policy不足で停止します。</td></tr>'}</tbody></table><div class="cell-main" style="padding:8px">Target Stock</div><table><thead><tr><th>需要地</th><th>資源</th><th>目標</th><th>優先度</th><th>操作</th></tr></thead><tbody>${targetRows||'<tr><td colspan="5">Target Stockなし</td></tr>'}</tbody></table>`;
  }
  ...
  function renderRequirements(){
    ...
    const policyChoices=(assigned)=>`<option value="" ${assigned==null?'selected':''}>Scenario global / 未割当</option>${(logistics().logistics_policies||[]).map((p)=>`<option value="${esc(p.id)}" ${p.id===assigned?'selected':''}>${esc(p.id)}</option>`).join('')}`;
    const rows=items.map((d)=>{const runway=d.local_runway_days==null?'—':`${fmt(d.local_runway_days,1)}日`,forecast=d.forecast_requirement_day==null?'—':`Day ${fmt(d.forecast_requirement_day,0)}`,arrival=d.earliest_confirmed_arrival_day==null?'—':`Day ${fmt(d.earliest_confirmed_arrival_day,0)}`,gap=d.projected_gap_days==null?'—':`${fmt(d.projected_gap_days,1)}日`,stateClass=['local_covered','pipeline_covered'].includes(d.supply_state)?'ok':'warn',draftKey=`supply-owner:${d.owner_kind}:${d.owner_id}:policy`;return `<tr><td><div class="cell-main">${esc(ownerLabel(d.owner_kind))}</div><div class="cell-sub">${esc(d.owner_id)}</div><div class="form-row"><label>物流Policy<select data-requirement-policy-select data-owner-kind="${esc(d.owner_kind)}" data-owner-id="${esc(d.owner_id)}" data-draft-key="${esc(draftKey)}">${policyChoices(d.assigned_policy_id)}</select></label><button type="button" data-requirement-policy-apply data-owner-kind="${esc(d.owner_kind)}" data-owner-id="${esc(d.owner_id)}">適用</button></div></td><td>${esc(resourceName(d.resource_id))}</td><td><div class="cell-main">${d.source_mode==='pinned'&&d.allowed_source_ids?.length?esc(locationName(d.allowed_source_ids[0])):d.preferred_source_id?`優先 ${esc(locationName(d.preferred_source_id))}`:d.source_mode==='allow_any'?'Policy委任':d.resolved_policy_id?'Policy指定':'Policy未解決'} → ${esc(locationName(d.destination_id))}</div><div class="cell-sub">${esc(d.resolved_policy_id||'policyなし')} · ${esc(policyLabel(d.path_preference||'balanced'))} · 経路候補 ${(d.path_candidates||[]).length}</div></td><td>${fmt(d.requested_t)} t<div class="cell-sub">現地 ${fmt(d.local_supply_t)} / 外部 ${fmt(d.external_required_t)} t</div></td><td>${esc(forecast)}</td><td>${fmt(d.pipeline_t)} t<div class="cell-sub">最短確定到着 ${esc(arrival)}</div></td><td>${fmt(d.remaining_t)} t<div class="cell-sub">猶予 ${esc(runway)} · gap ${esc(gap)}</div></td><td><span class="badge ${stateClass}">${esc(requirementStateLabel(d))}</span><div class="cell-sub">供給元 ${d.operational_source_count}/${d.candidate_source_count} · 在庫源 ${d.stocked_source_count}</div></td><td>${esc(priorityText(d.priority))}</td></tr>`;}).join('');
    $('#requirementTable').innerHTML=`<table><thead><tr><th>発生元 / Policy</th><th>資源</th><th>供給→需要地</th><th>要求/現地</th><th>必要時期</th><th>輸送系内</th><th>未充足/猶予</th><th>供給状態</th><th>優先</th></tr></thead><tbody>${rows||'<tr><td colspan="9">現在のSupply Requirementなし</td></tr>'}</tbody></table>`;
  }
  ...
  function populateLocationSelects(){
    ...
    const source=$('#logisticsPolicySource');if(source){const current=source.value;source.innerHTML='<option value="">—</option>'+opts;if([...source.options].some((o)=>o.value===current))source.value=current;}
  ...
  function openLogisticsPolicyDialog(movementPlan=null,policy=null){
    populateLocationSelects();
    editingLogisticsPolicyId=policy?.id||null;$('#logisticsPolicyId').disabled=Boolean(policy);$('#logisticsPolicyId').value=policy?.id||'';$('#logisticsPolicySourceMode').value=policy?.source_mode||(movementPlan?'pinned':'allow_any');$('#logisticsPolicySource').value=(policy?.source_mode==='pinned'?policy?.allowed_source_ids?.[0]:policy?.preferred_source_id)||movementPlan?.origin_id||'';$('#logisticsPolicyAllowedSources').value=(policy?.allowed_source_ids||[]).join(', ');$('#logisticsPolicyPathMode').value=policy?.path_mode||(movementPlan?'pinned':'allow_any');$('#logisticsPolicyPathPreference').value=policy?.path_preference||'balanced';$('#logisticsPolicyExplicitPath').value=(policy?.explicit_path|| (movementPlan?[movementPlan.id]:[])).join(', ');$('#logisticsPolicyAllowedHandoffs').value=(policy?.allowed_handoff_ids||[]).join(', ');$('#logisticsPolicyAllowedServices').value=(policy?.allowed_service_ids||[]).join(', ');$('#logisticsPolicyGlobal').checked=Boolean(policy?.is_global);$('#logisticsPolicyDialog').showModal();
  }
  ...
  document.addEventListener('click',async(event)=>{
    ...
    if(event.target.closest('#newLogisticsPolicyButton')){openLogisticsPolicyDialog();return;} if(event.target.closest('#newTargetStockButton')){openTargetStockDialog();return;} if(event.target.closest('#movementPlanLogisticsPolicyButton')){openLogisticsPolicyDialog((state.movementPlans?.items||[]).find((x)=>x.id===state.selectedMovementPlanId));return;}
    ...
    const policyEdit=event.target.closest('[data-logistics-policy-edit]');if(policyEdit){const row=(logistics().logistics_policies||[]).find((x)=>x.id===policyEdit.dataset.logisticsPolicyEdit);if(row)openLogisticsPolicyDialog(null,row);return;}const policyDelete=event.target.closest('[data-logistics-policy-delete]');if(policyDelete){try{await command('DeleteLogisticsPolicy',{policy_id:policyDelete.dataset.logisticsPolicyDelete});}catch{}return;}const globalSet=event.target.closest('[data-logistics-global-set]');if(globalSet){try{await command('SetGlobalLogisticsPolicy',{policy_id:globalSet.dataset.logisticsGlobalSet});}catch{}return;}if(event.target.closest('[data-logistics-global-clear]')){try{await command('SetGlobalLogisticsPolicy',{policy_id:null});}catch{}return;}
    const requirementPolicy=event.target.closest('[data-requirement-policy-apply]');if(requirementPolicy){const selector=document.querySelector(`[data-requirement-policy-select][data-owner-kind="${CSS.escape(requirementPolicy.dataset.ownerKind)}"][data-owner-id="${CSS.escape(requirementPolicy.dataset.ownerId)}"]`),policyId=selector?.value||null;try{await command(policyId?'AssignLogisticsPolicy':'UnassignLogisticsPolicy',policyId?{owner_kind:requirementPolicy.dataset.ownerKind,owner_id:requirementPolicy.dataset.ownerId,policy_id:policyId}:{owner_kind:requirementPolicy.dataset.ownerKind,owner_id:requirementPolicy.dataset.ownerId});}catch{}return;}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('#logisticsPolicyCloseButton').addEventListener('click',()=>$('#logisticsPolicyDialog').close());$('#logisticsPolicyCancelButton').addEventListener('click',()=>$('#logisticsPolicyDialog').close());
    $('#logisticsPolicyForm').addEventListener('submit',async(event)=>{event.preventDefault();const sourceMode=$('#logisticsPolicySourceMode').value,source=$('#logisticsPolicySource').value||null,pathMode=$('#logisticsPolicyPathMode').value,explicitPath=$('#logisticsPolicyExplicitPath').value.split(',').map((v)=>v.trim()).filter(Boolean),allowedSources=$('#logisticsPolicyAllowedSources').value.split(',').map((v)=>v.trim()).filter(Boolean),allowedHandoffs=$('#logisticsPolicyAllowedHandoffs').value.split(',').map((v)=>v.trim()).filter(Boolean),allowedServices=$('#logisticsPolicyAllowedServices').value.split(',').map((v)=>v.trim()).filter(Boolean);if(sourceMode==='pinned'&&source){allowedSources.splice(0,allowedSources.length,source);}const payload={policy_id:$('#logisticsPolicyId').value.trim(),source_mode:sourceMode,allowed_source_ids:allowedSources.length?allowedSources:null,preferred_source_id:sourceMode==='preferred'?source:null,path_mode:pathMode,explicit_path:pathMode==='pinned'?explicitPath:null,allowed_handoff_ids:allowedHandoffs.length?allowedHandoffs:null,allowed_service_ids:allowedServices.length?allowedServices:null,path_preference:pathMode==='allow_any'?'balanced':$('#logisticsPolicyPathPreference').value};try{await command(editingLogisticsPolicyId?'UpdateLogisticsPolicy':'CreateLogisticsPolicy',payload);if($('#logisticsPolicyGlobal').checked)await command('SetGlobalLogisticsPolicy',{policy_id:payload.policy_id});else if(editingLogisticsPolicyId&&(logistics().logistics_policies||[]).find((x)=>x.id===editingLogisticsPolicyId)?.is_global)await command('SetGlobalLogisticsPolicy',{policy_id:null});editingLogisticsPolicyId=null;$('#logisticsPolicyDialog').close();}catch{}});
  ...
  window.SpaceIdleLogistics={render,openLogisticsPolicyDialog,openTargetStockDialog,openAllocationDialog};
})();
