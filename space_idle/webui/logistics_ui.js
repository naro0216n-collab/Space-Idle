(() => {
  ...
  function renderAllocations(){
    ...
    const cards=items.map((a)=>{
      const issues=[...(a.blockers||[]),...(a.limiting_factors||[])].map(A.userFacingText);
  ...
  function renderVehicleProduction(){
    ...
    const projectCards=projects.map((p)=>{const blockers=(p.blockers||[]).map(A.userFacingText),toggle=p.phase==='complete'?'<span class="badge ok">完成</span>':`<button type="button" data-production-toggle="${esc(p.id)}" data-paused="${p.paused?'1':'0'}">${p.paused?'再開':'停止'}</button>`;return `<article class="vehicle-production-card ${isDecisionContext('vehicle_production',p.id)?'is-context-target':''} ${blockers.length?'has-warning':''}" data-production-project-row="${esc(p.id)}"><div class="decision-card-title"><span><strong>${esc(p.display_name)}</strong><small>${esc(locationName(p.operational_node_id))}</small></span><span class="badge ${p.phase==='complete'?'ok':blockers.length?'warn':''}">${p.phase==='complete'?'完成':p.paused?'停止中':'建造中'}</span></div><div class="vehicle-production-metrics"><span><small>進捗</small><strong>${fmt(p.progress_days,1)} / ${fmt(p.required_days,1)}日</strong></span><label>優先度<select data-production-priority-value data-draft-key="production:${esc(p.id)}:priority" ${p.priority_editable?'':'disabled'}>${priorityOptions(p.priority??3)}</select></label></div><div class="decision-card-footer ${blockers.length?'has-warning':''}">${blockers.length?`制約: ${esc(blockers[0])}`:'主要な制約なし'}</div><div class="action-row">${toggle}${p.phase==='complete'?'':`<button type="button" data-production-settings="${esc(p.id)}" ${p.priority_editable?'':'disabled'}>設定適用</button>`}</div></article>`;}).join('');
    const optionCards=options.map((o)=>{const blockers=(o.blockers||[]).map(A.userFacingText);return `<article class="vehicle-production-card ${blockers.length?'has-warning':''}" data-production-option-row><div class="decision-card-title"><span><strong>${esc(o.display_name)}</strong><small>${esc(locationName(o.operational_node_id))}</small></span><span class="badge ${o.can_plan?'ok':'warn'}">${o.can_plan?'建造可能':'条件不足'}</span></div><div class="vehicle-production-resource"><span>所要 ${fmt(o.production_days,1)}日</span><span>${(o.resources||[]).map(([rid,amount])=>`${esc(resourceName(rid))} ${fmt(amount)} t`).join(' / ')||'追加資源なし'}</span></div><label class="vehicle-production-priority">優先度<select data-production-priority-value data-draft-key="production-option:${esc(o.vehicle_definition_id)}:${esc(o.operational_node_id)}:priority">${priorityOptions(3)}</select></label><div class="decision-card-footer ${blockers.length?'has-warning':''}">${blockers.length?`制約: ${esc(blockers[0])}`:'主要な制約なし'}</div><button type="button" data-produce-vehicle="${esc(o.vehicle_definition_id)}" data-production-location="${esc(o.operational_node_id)}" ${o.can_plan?'':'disabled'}>建造を開始</button></article>`;}).join('');
