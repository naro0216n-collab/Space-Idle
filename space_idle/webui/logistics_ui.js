(() => {
  ...
  function renderAllocationServiceOptions(view=allocationOptionsView){
    ...
    root.innerHTML=`<h3>Transport Service候補</h3><div class="cell-sub">Vehicle性能と正準Movement評価からApplicationが導出した候補です。経路を固定する必要がある場合だけMovement hard constraintを指定します。</div>${cards||'<div class="empty-state">候補なし</div>'}`;
  }
  ...
  function renderSupplyPolicies(){
    ...
    const constraintRows=constraints.map((row)=>{
      const scope=[row.owner_kind&&row.owner_id?`${ownerLabel(row.owner_kind)}:${row.owner_id}`:'需要地全体',row.resource_id?resourceName(row.resource_id):'全Resource'].join(' / ');
      const via=(row.required_via_node_ids||[]).map(locationName).join(' → ')||'指定なし';
      const allocations=(row.required_transport_allocation_ids||[]).join(', ')||'指定なし';
      const key=constraintScopeKey(row);
      return `<tr><td><div class="cell-main">${esc(locationName(row.destination_id))}</div><div class="cell-sub">${esc(scope)}</div></td><td>${row.source_node_id?esc(locationName(row.source_node_id)):'自動'}</td><td><div class="cell-main">via ${esc(via)}</div><div class="cell-sub">Allocation ${esc(allocations)}</div></td><td><div class="action-row"><button type="button" data-routing-constraint-edit="${esc(key)}">編集</button><button type="button" class="danger-button" data-routing-constraint-clear="${esc(key)}">解除</button></div></td></tr>`;
    }).join('');
    ...
    $('#routingConstraintTable').innerHTML=`<div style="padding:8px" class="action-row"><button type="button" class="primary" id="newRoutingConstraintButton">Routing Constraintを設定</button><button type="button" id="newTargetStockButton">Target Stockを設定</button></div><div class="cell-main" style="padding:8px">Supply Routing Constraint</div><table><thead><tr><th>Scope</th><th>固定供給元</th><th>必須経路条件</th><th>操作</th></tr></thead><tbody>${constraintRows||'<tr><td colspan="4">Constraintなし。既存Network内からcanonical routingで自動選択します。</td></tr>'}</tbody></table><div class="cell-main" style="padding:8px">Target Stock</div><table><thead><tr><th>需要地</th><th>資源</th><th>目標</th><th>優先度</th><th>操作</th></tr></thead><tbody>${targetRows||'<tr><td colspan="5">Target Stockなし</td></tr>'}</tbody></table>`;
  }
  ...
  function renderFleet(){
    ...
    const rows=pools.map((p)=>`<tr data-fleet-pool-row><td><div class="cell-main">${esc(p.display_name)}</div><div class="cell-sub">${esc(p.vehicle_definition_id)}</div></td><td>${esc(locationName(p.operational_node_id))}</td><td>${fmt(p.total_units,0)}</td><td>${fmt(p.free_units,0)}</td><td>${fmt(p.transport_units,0)}</td><td>${fmt(p.exploration_units,0)}</td><td>${fmt(p.retirement_units,0)}</td><td>${fmt(p.other_committed_units,0)}</td><td>${fmt(p.relocating_units,0)}</td><td>${fmt(p.releasing_units,0)}</td><td><div class="action-row"><button type="button" data-fleet-relocate="${esc(p.vehicle_definition_id)}" data-fleet-source="${esc(p.operational_node_id)}" data-fleet-free="${fmt(p.free_units,0)}" ${Number(p.free_units||0)<=0?'disabled title="free Fleetなし"':''}>移動</button><input data-retirement-units type="number" min="1" max="${fmt(p.free_units,0)}" value="1" aria-label="退役unit数" ${Number(p.free_units||0)<=0?'disabled':''}><select data-retirement-priority aria-label="退役優先度" ${Number(p.free_units||0)<=0?'disabled':''}>${priorityOptions(3)}</select><button type="button" data-fleet-retire="${esc(p.vehicle_definition_id)}" data-fleet-retire-node="${esc(p.operational_node_id)}" ${Number(p.free_units||0)<=0?'disabled title="free Fleetなし"':''}>退役計画</button></div></td></tr>`).join('');
    ...
    const retirementRows=retirements.map((r)=>{const salvage=(r.expected_salvage||[]).map(([rid,amount])=>`${resourceName(rid)} ${fmt(amount,2)}t`).join(' / ')||'なし';const projected=(r.projected_salvage||[]).map(([rid,amount])=>`${resourceName(rid)} ${fmt(amount,2)}t`).join(' / ')||'なし';const actual=(r.actual_salvage||[]).map(([rid,amount])=>`${resourceName(rid)} ${fmt(amount,2)}t`).join(' / ')||'なし';const blockers=(r.blockers||[]).map(A.userFacingText).join(' / ')||'なし';return `<div class="detail-card" data-retirement-row="${esc(r.id)}"><div class="mode-title"><span>${esc(r.display_name)} ${fmt(r.units,0)} unit · ${esc(locationName(r.operational_node_id))}</span><span class="badge ${r.phase==='complete'?'ok':r.blockers?.length?'warn':''}">${esc(r.phase)}</span></div>${kv([['進捗',`${fmt(r.progress_work,1)} / ${fmt(r.required_work,1)} work`],['Recovery potential',esc(salvage)],['Projected recoverable fraction',pct(r.projected_salvage_fraction??1)],['Projected recovery',esc(projected)],['Actual recoverable fraction',r.actual_salvage_fraction==null?'—':pct(r.actual_salvage_fraction)],['Actual recovery',esc(actual)],['blocker',esc(blockers)],['不可逆',r.irreversible_started?'開始済み':'未開始']])}<div class="action-row"><select data-retirement-active-priority ${['complete','cancelled'].includes(r.phase)?'disabled':''}>${priorityOptions(r.priority??3)}</select><button type="button" data-retirement-priority-save="${esc(r.id)}" ${['complete','cancelled'].includes(r.phase)?'disabled':''}>優先度適用</button><button type="button" class="danger-button" data-retirement-cancel="${esc(r.id)}" ${r.irreversible_started||['complete','cancelled'].includes(r.phase)?'disabled title="不可逆開始後は取消不可"':''}>退役取消</button></div></div>`;}).join('');
    $('#vehicleTable').innerHTML=`<table><thead><tr><th>Vehicle type</th><th>所在地</th><th>総数</th><th>free</th><th>Transport</th><th>Exploration</th><th>Retirement</th><th>その他拘束</th><th>relocating</th><th>releasing</th><th>操作</th></tr></thead><tbody>${rows||'<tr><td colspan="11">Fleetなし</td></tr>'}</tbody></table>${retirementRows?`<div style="padding:8px"><strong>退役</strong>${retirementRows}</div>`:''}${relocationRows?`<div style="padding:8px"><strong>移動中</strong>${relocationRows}</div>`:''}${releaseRows?`<div style="padding:8px"><strong>回収中</strong>${releaseRows}</div>`:''}`;
  }
  ...
  function renderAllocations(){
    ...
    const rows=items.map((a)=>{const blocked=(a.blockers||[]).length+(a.limiting_factors||[]).length;const constraint=(a.movement_hard_constraint||[]).length?`固定: ${pathText(a.movement_hard_constraint)}`:'自動選択';const selected=`F ${pathText(a.selected_forward_path)}${(a.selected_reverse_path||[]).length?` / R ${pathText(a.selected_reverse_path)}`:''}`;return `<tr class="${isDecisionContext('transport_allocation',a.id)?'is-context-target':''}" data-allocation-row="${esc(a.id)}"><td><div class="cell-main">${esc(a.display_name)}</div><div class="cell-sub">${esc(locationName(a.anchor_node_id))} → ${esc(locationName(a.destination_id))} · ${esc(a.id)}</div><div class="cell-sub">Route: ${esc(selected)} · ${esc(constraint)}</div><div class="cell-sub">Infrastructure: ${esc(infrastructureText(a.infrastructure_requirements))}</div><div class="cell-sub">Resource: ${(a.operational_supply||[]).map(([loc,rid,amount])=>`${esc(locationName(loc))} ${esc(resourceName(rid))} ${fmt(amount,2)} t/日`).join(' / ')||'追加運用Resourceなし'}</div></td><td><div class="cell-main">${esc(allocationTarget(a))}</div><div class="cell-sub">方向別Capacity target</div></td><td>${fmt(a.active_units,0)} / ${fmt(a.required_units,0)}<div class="cell-sub">unfilled ${fmt(a.unfilled_units,0)}</div></td><td>${capText(a.nominal)}</td><td>${capText(a.available)}</td><td>${capText(a.used)}</td><td>${capText(a.spare)}</td><td>${fmt(a.cycle_days,1)}日<div class="cell-sub">latency ${fmt(a.forward_latency_days,0)}日</div></td><td>${blocked}<div class="cell-sub">${esc([...(a.blockers||[]),...(a.limiting_factors||[])].slice(0,2).map(A.userFacingText).join(' / '))}</div></td><td><div class="action-row"><button type="button" data-allocation-edit="${esc(a.id)}">設定</button><button type="button" data-allocation-toggle="${esc(a.id)}" data-paused="${a.paused?'1':'0'}">${a.paused?'再開':'停止'}</button><button type="button" class="danger-button" data-allocation-delete="${esc(a.id)}">削除</button></div></td></tr>`;}).join('');
    $('#allocationTable').innerHTML=`<div style="padding:8px"><button type="button" class="primary" id="newAllocationButton">Transport Allocationを作成</button></div><table><thead><tr><th>Service / route</th><th>Target Capacity</th><th>active / required</th><th>Nominal F/R</th><th>Available F/R</th><th>Used F/R</th><th>Spare F/R</th><th>cycle</th><th>blocker</th><th>操作</th></tr></thead><tbody>${rows||'<tr><td colspan="10">Transport Allocationなし。Fleetはfreeのままです。</td></tr>'}</tbody></table>`;
  }
  ...
  function populateLocationSelects(){
    ...
    for(const id of ['targetStockDestination','allocationSource','allocationDestination','relocationDestination','routingConstraintDestination']){const el=$('#'+id);if(el){const current=el.value;el.innerHTML=opts;if([...el.options].some((o)=>o.value===current))el.value=current;}}
    const source=$('#routingConstraintSource');if(source){const current=source.value;source.innerHTML='<option value="">指定なし</option>'+opts;if([...source.options].some((o)=>o.value===current))source.value=current;}
    ...
    const constraintResource=$('#routingConstraintResource');if(constraintResource){const current=constraintResource.value;constraintResource.innerHTML='<option value="">全Resource</option>'+resourceOpts;if([...constraintResource.options].some((o)=>o.value===current))constraintResource.value=current;}
  ...
  function openRoutingConstraintDialog(row=null,prefill=null){
    ...
    const value=row||prefill||{};editingRoutingConstraintScope=row?constraintScopePayload(row):null;
    $('#routingConstraintDestination').value=value.destination_id||'';
    $('#routingConstraintOwnerKind').value=value.owner_kind||'';
    $('#routingConstraintOwnerId').value=value.owner_id||'';
    $('#routingConstraintResource').value=value.resource_id||'';
    $('#routingConstraintSource').value=value.source_node_id||value.routing_constraint_source_id||'';
    $('#routingConstraintViaNodes').value=(value.required_via_node_ids||value.routing_constraint_via_node_ids||[]).join(', ');
    $('#routingConstraintAllocations').value=(value.required_transport_allocation_ids||value.routing_constraint_transport_allocation_ids||[]).join(', ');
  ...
  function openAllocationDialog(movementPlan=null,allocation=null){
    ...
    if(allocation){
      $('#allocationVehicle').value=allocation.vehicle_definition_id;$('#allocationSource').value=allocation.anchor_node_id;$('#allocationDestination').value=allocation.destination_id;$('#allocationPriority').value=String(allocation.provisioning_priority);$('#allocationForward').value=String(allocation.target_capacity?.forward_t_per_day??0);$('#allocationReverse').value=String(allocation.target_capacity?.reverse_t_per_day??0);$('#allocationMovementConstraint').value=(allocation.movement_hard_constraint||[]).join(', ');
    }else if(movementPlan){$('#allocationSource').value=movementPlan.origin_id;$('#allocationDestination').value=movementPlan.destination_id;$('#allocationMovementConstraint').value=movementPlan.id||'';}else{$('#allocationMovementConstraint').value='';}
    ...
    $('#allocationDialog').showModal();updateAllocationServiceOptions();
  }
  ...
  document.addEventListener('click',async(event)=>{
    ...
    if(event.target.closest('#newRoutingConstraintButton')){openRoutingConstraintDialog();return;} if(event.target.closest('#newTargetStockButton')){openTargetStockDialog();return;}
    ...
    const allocationOption=event.target.closest('[data-allocation-option]');if(allocationOption){$('#allocationVehicle').value=allocationOption.dataset.allocationOption;renderAllocationServiceOptions();return;}
    ...
    const retirementPriority=event.target.closest('[data-retirement-priority-save]');if(retirementPriority){const row=retirementPriority.closest('[data-retirement-row]');try{await command('SetFleetRetirementPriority',{retirement_id:retirementPriority.dataset.retirementPrioritySave,priority:Number(row.querySelector('[data-retirement-active-priority]').value)});}catch{}return;}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('#routingConstraintCloseButton').addEventListener('click',()=>{editingRoutingConstraintScope=null;$('#routingConstraintDialog').close();});$('#routingConstraintCancelButton').addEventListener('click',()=>{editingRoutingConstraintScope=null;$('#routingConstraintDialog').close();});
    $('#routingConstraintForm').addEventListener('submit',async(event)=>{event.preventDefault();const ownerKind=$('#routingConstraintOwnerKind').value.trim()||null,ownerId=$('#routingConstraintOwnerId').value.trim()||null;if(Boolean(ownerKind)!==Boolean(ownerId)){banner('Owner kind と Owner ID は両方指定するか両方空欄にしてください','error');return;}const split=(value)=>value.split(',').map((v)=>v.trim()).filter(Boolean);const payload={destination_id:$('#routingConstraintDestination').value,owner_kind:ownerKind,owner_id:ownerId,resource_id:$('#routingConstraintResource').value||null,source_node_id:$('#routingConstraintSource').value||null,required_via_node_ids:split($('#routingConstraintViaNodes').value),required_transport_allocation_ids:split($('#routingConstraintAllocations').value)};if(!payload.source_node_id&&!payload.required_via_node_ids.length&&!payload.required_transport_allocation_ids.length){banner('供給元、必須経由Node、必須Transport Allocationのいずれかを指定してください','error');return;}try{if(editingRoutingConstraintScope){const oldKey=constraintScopeKey(editingRoutingConstraintScope),newKey=constraintScopeKey(payload);if(oldKey!==newKey)await command('ClearSupplyRoutingConstraint',editingRoutingConstraintScope);}await command('SetSupplyRoutingConstraint',payload);editingRoutingConstraintScope=null;$('#routingConstraintDialog').close();}catch{}});
