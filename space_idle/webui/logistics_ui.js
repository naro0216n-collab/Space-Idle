(() => {
  ...
  function renderVehicleProduction(){
    ...
    const projectRows=projects.map((p)=>{
      ...
      return `<tr data-production-project-row="${esc(p.id)}"><td><div class="cell-main">${esc(p.display_name)}</div><div class="cell-sub">${esc(p.id)} · ${esc(locationName(p.location_id))}</div></td><td>${esc(stateLabels[p.phase]||p.phase)}</td><td>${fmt(p.progress_days,1)}/${fmt(p.required_days,1)}日</td><td>${(p.resources||[]).map(([rid,amount])=>`${esc(resourceName(rid))} ${fmt(amount)}t`).join(' / ')}</td><td><input type="number" step="1" value="${fmt(p.priority,0)}" data-production-priority-value ${p.priority_editable?'':'disabled'} aria-label="Vehicle建造資材優先度"></td><td><input type="number" min="0.01" step="0.1" value="${fmt(p.allocation_weight,2)}" data-production-allocation-value ${p.allocation_editable?'':'disabled'} aria-label="Vehicle建造能力配分"></td><td>${blockers.length}<div class="cell-sub">${blockers.slice(0,2).map(A.userFacingText).join(' / ')}</div></td><td><div class="action-row">${controls}</div></td></tr>`;
    }).join('');
    const optionRows=options.map((o)=>{
      const blockers=o.blockers||[];
      return `<tr data-production-option-row><td><div class="cell-main">${esc(o.display_name)}</div><div class="cell-sub">${esc(locationName(o.location_id))}</div></td><td>${fmt(o.production_days,1)}日</td><td>${(o.resources||[]).map(([rid,amount])=>`${esc(resourceName(rid))} ${fmt(amount)}t`).join(' / ')}</td><td>${esc(o.production_capability_id||'—')}</td><td><input type="number" step="1" value="50" data-production-priority-value aria-label="Vehicle建造初期資材優先度"></td><td><input type="number" min="0.01" step="0.1" value="1" data-production-allocation-value aria-label="Vehicle建造初期能力配分"></td><td>${blockers.length}<div class="cell-sub">${blockers.slice(0,2).map(A.userFacingText).join(' / ')}</div></td><td><button type="button" data-produce-vehicle="${esc(o.vehicle_definition_id)}" data-production-location="${esc(o.location_id)}" ${blockers.length?'disabled':''}>建造</button></td></tr>`;
    }).join('');
