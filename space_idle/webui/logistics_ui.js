(() => {
  ...
  function renderFleet(){
    ...
    const retirementRows=retirements.map((r)=>{const salvage=(r.expected_salvage||[]).map(([rid,amount])=>`${resourceName(rid)} ${fmt(amount,2)}t`).join(' / ')||'なし';const blockers=(r.blockers||[]).map(A.userFacingText).join(' / ')||'なし';return `<div class="detail-card" data-retirement-row="${esc(r.id)}"><div class="mode-title"><span>${esc(r.display_name)} ${fmt(r.units,0)} unit · ${esc(locationName(r.operational_node_id))}</span><span class="badge ${r.phase==='complete'?'ok':r.blockers?.length?'warn':''}">${esc(r.phase)}</span></div>${kv([['進捗',`${fmt(r.progress_work,1)} / ${fmt(r.required_work,1)} work`],['Salvage',esc(salvage)],['blocker',esc(blockers)],['不可逆',r.irreversible_started?'開始済み':'未開始']])}<div class="action-row"><select data-retirement-active-priority ${['complete','cancelled'].includes(r.phase)?'disabled':''}>${priorityOptions(r.priority??3)}</select><button type="button" data-retirement-priority-save="${esc(r.id)}" ${['complete','cancelled'].includes(r.phase)?'disabled':''}>優先度適用</button><button type="button" class="danger-button" data-retirement-cancel="${esc(r.id)}" ${r.irreversible_started||['complete','cancelled'].includes(r.phase)?'disabled title="不可逆開始後は取消不可"':''}>退役取消</button></div></div>`;}).join('');
