(() => {
  ...
  const {state,$,esc,fmt,locationName,resourceName,definitionName,operationName,issueHtml,metricHtml,command,banner}=A;
  ...
  function renderFleet(){
    const pools=state.fleet?.pools||logistics().fleet_pools||[]; const relocations=state.fleet?.relocations||logistics().relocations||[];
    ...
    $('#vehicleTable').innerHTML=`<table><thead><tr><th>Vehicle type</th><th>所在地</th><th>総数</th><th>free</th><th>Transport</th><th>Exploration</th><th>relocating</th><th>releasing</th><th>操作</th></tr></thead><tbody>${rows||'<tr><td colspan="9">Fleetなし</td></tr>'}</tbody></table>${relocationRows?`<div style="padding:8px"><strong>移動中</strong>${relocationRows}</div>`:''}`;
  }
  ...
  function renderAllocations(){
    ...
    const rows=items.map((a)=>{const blocked=(a.blockers||[]).length+(a.limiting_factors||[]).length;return `<tr data-allocation-row="${esc(a.id)}"><td><div class="cell-main">${esc(a.display_name)}</div><div class="cell-sub">${esc(locationName(a.anchor_location_id))} → ${esc(locationName(a.destination_id))} · ${esc(a.id)}</div></td><td><span class="badge">${esc(a.control_mode.toUpperCase())}</span><div class="cell-sub">正本: ${esc(allocationTarget(a))}</div></td><td>${fmt(a.active_units,0)} / ${fmt(a.required_units,0)}<div class="cell-sub">unfilled ${fmt(a.unfilled_units,0)}</div></td><td>${capText(a.nominal)}</td><td>${capText(a.available)}</td><td>${capText(a.used)}</td><td>${capText(a.spare)}</td><td>${fmt(a.cycle_days,1)}日<div class="cell-sub">latency ${fmt(a.forward_latency_days,0)}日</div></td><td>${blocked}<div class="cell-sub">${esc([...(a.blockers||[]),...(a.limiting_factors||[])].slice(0,2).map(A.userFacingText).join(' / '))}</div></td><td><div class="action-row"><button type="button" data-allocation-edit="${esc(a.id)}">設定</button><button type="button" data-allocation-mode="${esc(a.id)}" data-mode="${esc(a.control_mode)}">${a.control_mode==='units'?'CAPACITYへ':'UNITSへ'}</button><button type="button" data-allocation-toggle="${esc(a.id)}" data-paused="${a.paused?'1':'0'}">${a.paused?'再開':'停止'}</button><button type="button" class="danger-button" data-allocation-delete="${esc(a.id)}">削除</button></div></td></tr>`;}).join('');
  ...
  function renderRouteInspector(){
    ...
    const modes=(route.modes||[]).map((m)=>`<div class="route-mode-card ${m.service_feasible?'is-usable':''}"><div class="mode-title"><span>${esc(m.display_name)}</span><span class="badge ${m.service_feasible?'ok':'warn'}">${m.service_feasible?'Service可':'阻害'}</span></div><div class="cell-sub">${m.kind==='external_service'?'外部Service':`Fleet total ${fmt(m.fleet_total_units,0)} / free ${fmt(m.fleet_free_units,0)}`} · nominal ${capText(m.nominal_capacity)} · cycle ${m.cycle_days==null?'—':fmt(m.cycle_days,1)+'日'}</div>${(m.blockers||[]).length?`<div class="issue-stack">${m.blockers.map((b)=>issueHtml(['transport',b])).join('')}</div>`:''}</div>`).join('');
