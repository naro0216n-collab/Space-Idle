(() => {
  ...
  function renderVehicleProduction(){
    ...
    const projectRows=projects.map((p)=>{const blockers=p.blockers||[],toggle=p.phase==='complete'?'<span class="badge ok">完成</span>':`<button type="button" data-production-toggle="${esc(p.id)}" data-paused="${p.paused?'1':'0'}">${p.paused?'再開':'停止'}</button>`;return `<tr data-production-project-row="${esc(p.id)}"><td><div class="cell-main">${esc(p.display_name)}</div><div class="cell-sub">${esc(locationName(p.operational_node_id))}</div></td><td>${esc(p.phase)}</td><td>${fmt(p.progress_days,1)}/${fmt(p.required_days,1)}日</td><td><input type="number" step="1" value="${fmt(p.priority,0)}" data-production-priority-value data-draft-key="production:${esc(p.id)}:priority" ${p.priority_editable?'':'disabled'}></td><td>${blockers.length}<div class="cell-sub">${esc(blockers.slice(0,2).map(A.userFacingText).join(' / '))}</div></td><td><div class="action-row">${toggle}${p.phase==='complete'?'':`<button type="button" data-production-settings="${esc(p.id)}">設定適用</button>`}</div></td></tr>`;}).join('');
