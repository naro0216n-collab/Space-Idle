(() => {
  ...
  function renderNetwork(){
    ...
    svg.innerHTML=movementPlans.map((r)=>{const a=positions[r.origin_id],b=positions[r.destination_id];if(!a||!b)return'';const coords=`x1="${a[0]*9}" y1="${a[1]*4.7}" x2="${b[0]*9}" y2="${b[1]*4.7}"`;return `<line ${coords} class="network-line-hit" data-movement-plan-line="${esc(r.id)}" /><line ${coords} class="network-line ${r.service_feasible_now?'available':''} ${r.id===state.selectedMovementPlanId?'selected':''}" aria-hidden="true" />`;}).join('');
  ...
  function renderAllocations(){
    ...
    const rows=items.map((a)=>{const blocked=(a.blockers||[]).length+(a.limiting_factors||[]).length;const constraint=(a.movement_hard_constraint||[]).length?`固定: ${pathText(a.movement_hard_constraint)}`:'自動選択';const selected=`F ${pathText(a.selected_forward_path)}${(a.selected_reverse_path||[]).length?` / R ${pathText(a.selected_reverse_path)}`:''}`;return `<tr class="${isDecisionContext('transport_allocation',a.id)?'is-context-target':''}" data-allocation-row="${esc(a.id)}"><td><div class="cell-main">${esc(a.display_name)}</div><div class="cell-sub">${esc(locationName(a.anchor_node_id))} → ${esc(locationName(a.destination_id))}</div><div class="cell-sub">経路: ${esc(selected)} · ${esc(constraint)}</div><div class="cell-sub">必要インフラ: ${esc(infrastructureText(a.infrastructure_requirements))}</div><div class="cell-sub">運用資源: ${(a.operational_supply||[]).map(([loc,rid,amount])=>`${esc(locationName(loc))} ${esc(resourceName(rid))} ${fmt(amount,2)} t/日`).join(' / ')||'追加運用資源なし'}</div></td><td><div class="cell-main">${esc(allocationTarget(a))}</div><div class="cell-sub">方向別能力目標</div></td><td>${fmt(a.active_units,0)} / ${fmt(a.required_units,0)}<div class="cell-sub">不足 ${fmt(a.unfilled_units,0)}</div></td><td>${capText(a.nominal)}</td><td>${capText(a.available)}</td><td>${capText(a.used)}</td><td>${capText(a.spare)}</td><td>${fmt(a.cycle_days,1)}日<div class="cell-sub">片道 ${fmt(a.forward_latency_days,0)}日</div></td><td>${blocked}<div class="cell-sub">${esc([...(a.blockers||[]),...(a.limiting_factors||[])].slice(0,2).map(A.userFacingText).join(' / '))}</div></td><td><div class="action-row"><button type="button" data-allocation-edit="${esc(a.id)}">設定</button><button type="button" data-allocation-toggle="${esc(a.id)}" data-paused="${a.paused?'1':'0'}">${a.paused?'再開':'停止'}</button><button type="button" class="danger-button" data-allocation-delete="${esc(a.id)}">削除</button></div></td></tr>`;}).join('');
    $('#allocationTable').innerHTML=`<div style="padding:8px"><button type="button" class="primary" id="newAllocationButton">輸送能力を設定</button></div><table><thead><tr><th>Service / route</th><th>Target Capacity</th><th>active / required</th><th>Nominal F/R</th><th>Available F/R</th><th>Used F/R</th><th>Spare F/R</th><th>cycle</th><th>blocker</th><th>操作</th></tr></thead><tbody>${rows||'<tr><td colspan="10">輸送能力設定なし。Fleetはfreeのままです。</td></tr>'}</tbody></table>`;
  }
  ...
  function renderMovementPlanInspector(){
    ...
    content.innerHTML=section('区間',kv(segmentRows))+section('必要移動操作',(movementPlan.operations||[]).map((o)=>`<span class="badge">${esc(operationName(Array.isArray(o)?o[0]:o))}</span>`).join(' ')||'—')+section('現在のblocker',(movementPlan.blockers||[]).length?`<div class="issue-stack">${movementPlan.blockers.map((b)=>issueHtml(['transport',b])).join('')}</div>`:'<span class="badge ok">なし</span>')+section('輸送手段候補',modes||'<div class="empty-state">候補なし</div>')+section('操作','<div class="action-stack"><button type="button" class="primary" id="movementPlanAllocationButton">この区間へFleetを配備</button></div>');
  }
