(() => {
  ...
  const stateLabels = {
    ...
    in_transit:'輸送中', arrival_waiting:'到着待機', theory:'理論研究', prototype:'試作', demonstration:'実証',
  };
  ...
  function applyUiSnapshot(data) {
    ...
    state.globalIssues = data.global_issues;
    state.research = data.research;
    state.contracts = data.contracts;
    state.logisticsSummary = data.logistics_summary;
  ...
  function renderHeader() {
    ...
    const paused = Boolean(state.session?.time_paused);
    const speed = Number(state.session?.time_speed_multiplier || 1);
    const pause = $('#timePauseButton');
    pause.textContent = paused ? '▶ 再開' : '⏸ 一時停止';
    pause.setAttribute('aria-label', paused ? '再開' : '一時停止');
    pause.setAttribute('aria-pressed', paused ? 'true' : 'false');
    $$('[data-time-speed]').forEach((button) => {
      const selected = Number(button.dataset.timeSpeed) === speed;
      button.setAttribute('aria-pressed', selected ? 'true' : 'false');
      button.disabled = selected;
    });
    const timeState = $('#timeState');
    if (timeState) timeState.textContent = state.session?.automatic_progress_enabled === false ? '自動進行無効' : paused ? `停止中 · ${speed}×` : `自動進行 · ${speed}×`;
  }

  function renderLocations() {
    $('#locationList').innerHTML = (state.world?.locations || []).map((loc) => `
  ...
  function renderOverviewTab() {
    ...
    return `<div class="card-grid">
      <section class="card"><div class="card-heading"><h3>運用状態</h3></div><div class="card-body"><div class="stat-grid">${statHtml('電力利用率',pct(flow?.power_utilization))}${statHtml('利用可能電力',`${fmt(loc.power_allocated_mw)} MW`)}${statHtml('建設能力',`${fmt(loc.construction_capacity_per_day)} /日`)}${statHtml('進行中建設',`${loc.projects?.length || 0}`)}</div></div></section>
  ...
  function researchBlockers(r) {
    if (r.status === 'prototype') return r.prototype_blockers || [];
    if (r.status === 'demonstration') return r.demonstration_blockers || [];
    return r.theory_blockers || [];
  }

  function researchProgress(r) {
    if (r.status === 'demonstration') return [r.demonstration_done_days, r.demonstration_required_days, '実証日'];
    return [r.theory_done, r.theory_required, 'Theory'];
  }
  ...
  function renderContractsTab() {
    const rows = (state.contracts?.items || []).map((c) => `<tr class="selectable" data-inspect="contract" data-id="${esc(c.id)}"><td><div class="cell-main">${esc(c.display_name)}</div><div class="cell-sub">${c.kind === 'cargo' ? '貨物契約' : '能力契約'}</div></td><td>${esc(stateLabels[c.status] || c.status)}</td><td>Day ${c.deadline_day}</td><td>${fmt(c.reward_musd)} M$</td><td>${(c.blockers || []).length}</td></tr>`).join('');
    return `<section class="card"><div class="card-heading"><h3>契約</h3></div><div class="table-wrap"><table><thead><tr><th>契約</th><th>状態</th><th>期限</th><th>報酬</th><th>blocker</th></tr></thead><tbody>${rows || '<tr><td colspan="5">契約なし</td></tr>'}</tbody></table></div></section>`;
  }
  ...
  function renderResourceInspector(id) {
    const inv=state.location?.inventory.find((x)=>x.resource_id===id), f=state.flow?.resources.find((x)=>x.resource_id===id); if(!inv)return;
  ...
  function siteOptionsHtml(r, kind) {
    const options = kind === 'prototype' ? (r.prototype_sites || []) : (r.demonstration_sites || []);
    if (!options.length) return '<div class="empty-state">候補地点なし</div>';
    return options.map((site) => {
      const blocked = (site.blockers || []).length;
      const attr = kind === 'prototype' ? 'data-research-prototype-site' : 'data-research-demo-site';
      return `<div class="route-mode-card"><div class="mode-title"><span>${esc(locationName(site.location_id))}</span><span class="badge ${blocked?'warn':'ok'}">${blocked?`${blocked} blocker`:'実行可'}</span></div>${blocked?`<div class="issue-stack">${site.blockers.map((x)=>issueHtml(['research',x[1]||x])).join('')}</div>`:''}<button type="button" ${attr}="${esc(site.location_id)}" data-id="${esc(r.id)}" ${blocked?'disabled':''}>${kind==='prototype'?'この地点で試作':'この地点で実証'}</button></div>`;
    }).join('');
  }
  ...
  function renderResearchInspector(id) {
    ...
    const phaseBlockers=researchBlockers(r);
    let phaseAction='';
    if(r.status==='prototype') {
      ...
      phaseAction=section('試作',`<div class="cell-sub">必要資材: ${resources}</div>${siteOptionsHtml(r,'prototype')}`);
    } else if(r.status==='demonstration') {
      phaseAction=section('実証',`<div class="cell-sub">進捗 ${r.demonstration_done_days}/${r.demonstration_required_days}日</div>${siteOptionsHtml(r,'demonstration')}`);
    }
    const allocation = r.status==='complete' ? '' : `<div class="form-row"><label>研究配分<input id="researchWeightInput" type="number" min="0" step="0.1" value="${r.allocation_weight||1}"></label><button type="button" data-set-research-weight="${esc(r.id)}">配分を適用</button></div>`;
    setInspector(r.display_name, section('状態',kv([['段階',esc(stateLabels[r.status]||r.status)],['Theory',`${fmt(r.theory_done)}/${fmt(r.theory_required)}`],['配分',r.status==='complete'?'—':fmt(r.allocation_weight,2)],['適格能力/日',fmt(r.eligible_capacity_points_per_day,2)],['実証日数',`${r.demonstration_done_days}/${r.demonstration_required_days}`]]))+section('前提',(r.prerequisites||[]).length?(r.prerequisites||[]).map((x)=>`<span class="badge">${esc(definitionName(x))}</span>`).join(' '):'<span class="badge ok">なし</span>')+section('現在のblocker',phaseBlockers.length?`<div class="issue-stack">${phaseBlockers.map((x)=>issueHtml(['research',x[1]||x])).join('')}</div>`:'<span class="badge ok">なし</span>')+phaseAction+section('操作',`<div class="action-stack">${action}${allocation}</div>`));
  }
  ...
  function renderContractInspector(id) {
    ...
    const linked=c.cargo_order_id?(state.orders?.items||[]).find((o)=>o.id===c.cargo_order_id):null;
    let actions='<span class="badge">処理済み</span>';
    if(c.status==='offered') actions=`<div class="action-row"><button class="primary" data-contract-action="accept" data-id="${esc(c.id)}">受諾</button><button class="danger-button" data-contract-action="decline" data-id="${esc(c.id)}">辞退</button></div>`;
    else if(c.status==='accepted'&&c.kind==='cargo'&&!c.cargo_order_id) actions=`<button class="primary" data-contract-dispatch="${esc(c.id)}">契約貨物の輸送計画</button>`;
    const cargoState=c.kind==='cargo'?section('契約貨物',linked?kv([['輸送状態',esc(stateLabels[linked.status]||linked.status)],['配送済み',`${fmt(linked.delivered_t)}/${fmt(linked.amount_t)} t`],['輸送待ち',`${fmt(linked.waiting_t)} t`],['輸送中',`${fmt(linked.in_transit_t)} t`]]):'<div class="empty-state">輸送未設定</div>'):'';
    setInspector(c.display_name,section('契約',kv([['種別',c.kind==='cargo'?'貨物契約':'能力契約'],['状態',esc(stateLabels[c.status]||c.status)],['期限',`Day ${c.deadline_day}`],['報酬',`${fmt(c.reward_musd)} M$`],['出発地',esc(locationName(c.source_id))],['到着地',esc(locationName(c.destination_id))],['貨物',c.resource_id?`${esc(resourceName(c.resource_id))} ${fmt(c.cargo_t)}t`:'—']]))+cargoState+section('Blocker',(c.blockers||[]).length?`<div class="issue-stack">${c.blockers.map((x)=>issueHtml(['contract',x])).join('')}</div>`:'<span class="badge ok">なし</span>')+section('操作',actions));
  }
  ...
  function renderRouteFilters() {
    ...
    for(const [selId,label] of [['#routeOriginFilter','全出発地'],['#routeDestinationFilter','全到着地']]) {
      const sel=$(selId), current=sel.value;
  ...
  function renderCargo() {
    const items=(state.orders?.items||[]).filter((o)=>o.owner_kind!=='contract');
    $('#cargoCountBadge').textContent=`${items.length}件`;
    ...
    $('#cargoTable').innerHTML=`<div style="padding:8px"><button type="button" class="primary" id="newCargoButton">資源輸送を設定</button></div><table><thead><tr><th>資源</th><th>区間</th><th>配送済み/総量</th><th>状態</th><th>blocker</th></tr></thead><tbody>${rows||'<tr><td colspan="5">資源輸送なし</td></tr>'}</tbody></table>`;
  }
  ...
  function modeLabel(mode) {
    return mode.kind==='external_service' ? `外部サービス: ${mode.display_name}` : `ヴィークル: ${mode.display_name} (${mode.available_vehicle_count}機利用可)`;
  }

  async function loadCargoPlans() {
    const source=$('#cargoSource').value, destination=$('#cargoDestination').value;
    const planSelect=$('#cargoPlan'), legs=$('#cargoLegChoices');
    if(!source||!destination||source===destination){state.cargoPlans=null;planSelect.innerHTML='<option value="">出発地と到着地を指定</option>';legs.innerHTML='';return;}
    const plans=await api(`/api/v1/transport-plans?source_id=${encodeURIComponent(source)}&destination_id=${encodeURIComponent(destination)}`);
    state.cargoPlans=plans;
    const options=plans?.options||[];
    ...
    const policy=$('#cargoPolicy').value;
    const preferred=options.findIndex((p)=>p.policy===policy);
    if(preferred>=0)planSelect.value=String(preferred);
    renderCargoLegChoices();
  }

  function renderCargoLegChoices() {
    const options=state.cargoPlans?.options||[];
    const plan=options[Number($('#cargoPlan').value||0)];
    ...
    $('#cargoLegChoices').innerHTML=(plan.path||[]).map((routeId,index)=>{
      const route=(state.routes?.items||[]).find((r)=>r.id===routeId);
      if(!route)return'';
      const modes=route.modes||[];
      const opts=modes.map((m)=>`<option value="${esc(m.id)}" ${m.id===defaults[routeId]?'selected':''} ${!m.usable_now?'disabled':''}>${esc(modeLabel(m))}${m.usable_now?'':' — 利用不可'}</option>`).join('');
      return `<div class="route-mode-card"><div class="mode-title"><span>Leg ${index+1}: ${esc(route.display_name)}</span></div><label>輸送方式 / ヴィークル<select data-cargo-route-mode="${esc(routeId)}">${opts}</select></label></div>`;
    }).join('');
  }
  ...
  async function openCargoDialog(route=null, contract=null) {
    ...
    if(contract){
      ...
      $('#cargoSource').value=route.origin_id; $('#cargoDestination').value=route.destination_id;
    }
    $('#cargoDialog').showModal();
    try{await loadCargoPlans();}catch(e){banner(e.message,'error');}
  }

  function renderAll() {
    renderHeader(); renderLocations(); renderGlobalIssues();
    if(state.activeView==='operations') renderOperations(); else renderLogistics();
  }

  document.addEventListener('click', async (event) => {
    const viewBtn=event.target.closest('[data-view]');
    if(viewBtn){state.activeView=viewBtn.dataset.view;$$('.view-button').forEach((b)=>b.classList.toggle('is-active',b===viewBtn));$('#operationsView').hidden=state.activeView!=='operations';$('#logisticsView').hidden=state.activeView!=='logistics';renderAll();return;}
  ...
  $('#cargoSource').addEventListener('change',()=>loadCargoPlans().catch((e)=>banner(e.message,'error')));
  ...
  $('#cargoForm').addEventListener('submit',async(event)=>{
    ...
    const options=state.cargoPlans?.options||[], plan=options[Number($('#cargoPlan').value||0)];
    if(!plan){banner('実行可能な輸送計画を選択してください','error');return;}
    const routeModes=$$('[data-cargo-route-mode]').map((sel)=>[sel.dataset.cargoRouteMode,sel.value]);
    try{
      if(state.cargoContractId){
        await command('DispatchContractCargo',{contract_id:state.cargoContractId,path:plan.path,route_modes:routeModes});
      }else{
        await command('SubmitCargo',{source_id:$('#cargoSource').value,destination_id:$('#cargoDestination').value,resource_id:$('#cargoResource').value,amount_t:Number($('#cargoAmount').value),priority:Number($('#cargoPriority').value),path:plan.path,route_modes:routeModes,path_policy:$('#cargoPolicy').value});
      }
      $('#cargoDialog').close(); state.cargoContractId=null; banner('輸送を登録しました');
    }catch{}
  });
  ...
  window.setInterval(()=>{
    if(document.hidden||state.busy||$('#app')?.getAttribute('aria-busy')!=='false')return;
    loadUiSnapshot().catch((err)=>{setConnection('error','同期失敗');console.error(err);});
  },1000);
