(() => {
  ...
  function renderFacilitiesTab(){
    const rows=(state.location?.facilities||[]).map((f)=>`<tr class="selectable" data-inspect="facility" data-id="${esc(f.id)}"><td><div class="cell-main">${esc(f.display_name)}</div><div class="cell-sub">${esc(f.id)}</div></td><td>Lv ${fmt(f.level,0)}</td><td>${f.paused?'<span class="badge warn">停止</span>':'<span class="badge ok">稼働</span>'}</td><td>${pct(f.power_utilization)}</td><td>${f.power_priority??'—'}</td><td>${(f.activation_blockers||[]).length}</td></tr>`).join('');
    return `<section class="card"><div class="card-heading"><h3>設備一覧</h3><span class="badge">${state.location?.facilities.length||0}</span></div><div class="table-wrap"><table><thead><tr><th>設備</th><th>Level</th><th>状態</th><th>電力</th><th>優先度</th><th>blocker</th></tr></thead><tbody>${rows||'<tr><td colspan="6">設備なし</td></tr>'}</tbody></table></div></section>`;
  }
  ...
  function renderFacilityInspector(id){
    ...
    const provider=(state.research?.providers||[]).find((row)=>row.facility_id===f.id);
    const researchRows=provider?[
      ['Research Tier',fmt(provider.tier,0)],['RP生成',`${fmt(provider.generation_points_per_day,2)}/日`],['RP貯蔵',fmt(provider.storage_capacity_points,1)],
    ]:[];
    setInspector(f.display_name,section('状態',kv([['ID',esc(f.id)],['定義',esc(f.definition_id)],['Level',fmt(f.level,0)],['運転',f.paused?'手動停止':'稼働'],['電力利用率',pct(f.power_utilization)],['電力優先度',esc(f.power_priority??'—')],...researchRows]))+section('Blocker',blockers.length?`<div class="issue-stack">${blockers.map(issueHtml).join('')}</div>`:'<div class="badge ok">なし</div>')+section('操作',`<div class="action-stack"><button type="button" data-command="${f.paused?'ResumeFacility':'PauseFacility'}" data-facility-id="${esc(f.id)}">${f.paused?'設備を再開':'設備を停止'}</button><div class="form-row"><label>電力優先度<input id="facilityPriorityInput" type="number" step="1" value="${f.power_priority??50}"></label><button type="button" data-set-power-priority="${esc(f.id)}">優先度を適用</button></div></div>`));
  }
