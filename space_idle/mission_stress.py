...
class MissionPlanner:
    def plan(self, vehicle: VehicleCapability, option: TransferOption, payload_t: float) -> MissionPlan:
        ...
        if vehicle.max_payload_t < 0 or vehicle.max_transfer_energy < 0 or vehicle.endurance_days < 0:
        ...
        if option.duration_days > vehicle.endurance_days:
            raise ValueError("operation duration exceeds vehicle endurance")
        return MissionPlan(
            option.origin_id,
