    ...
    def advance_procurement(self, day: int) -> None:
        ...
        for project in ordered:
            ...
            if self.project_site_failures(
                ...
                continue
            if any(blocker.code.startswith("upgrade_") for blocker in self.blockers(project.id, day)):
                continue
                    ...
                    if local_met or waited >= wait_limit:
                        ...
                        if import_amount > 1e-9:
                            ...
                            if not self.logistics.can_submit(
                                project.import_source_id, project.location_id,
                                component.import_resource_id, import_amount, day,
                ...
                if state.import_committed_t is not None and state.import_committed_t > 1e-9:
                    ...
                    if need_import_reserve > 1e-9:
                        got = self.inventory.reserve(
                            ...
                            component.import_resource_id, need_import_reserve,
                        )
